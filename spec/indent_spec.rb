...
# lines.
describe ProGrammar::Indent do
  before do
    @indent = ProGrammar::Indent.new(ProGrammar.new)
  end
    ...
    test.lines.each_with_index do |line, i|
      ...
      if result == ""
        it "should fail to parse nesting on line #{i + 1} of example_nesting.rb" do
          expect { ProGrammar::Indent.nesting_at(test, i + 1) }
            .to raise_error ProGrammar::Indent::UnparseableNestingError
        end
      else
        it "should parse nesting on line #{i + 1} of example_nesting.rb" do
          # rubocop:disable Security/Eval
          expect(ProGrammar::Indent.nesting_at(test, i + 1)).to eq eval(result)
