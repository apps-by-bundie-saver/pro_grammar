...
describe ProGrammar::WrappedModule do
  describe "#initialize" do
    it "should raise an exception when a non-module is passed" do
      expect { ProGrammar::WrappedModule.new(nil) }.to raise_error ArgumentError
    end
  end
  ...
  describe "candidates" do
    ...
    describe "number_of_candidates" do
      it 'should return the correct number of candidates' do
        expect(ProGrammar::WrappedModule(Host::CandidateTest).number_of_candidates).to eq 3
      end

      it 'should return 0 candidates for a class with no nested modules or methods' do
        expect(ProGrammar::WrappedModule(Host::PitifullyBlank).number_of_candidates).to eq 0
      end

      it 'should return 1 candidate for a class with a nested module with methods' do
        expect(ProGrammar::WrappedModule(Host::ForeverAlone).number_of_candidates).to eq 1
      end
    end

    describe "ordering of candidates" do
      it 'should return class with largest number of methods as primary candidate' do
        expect(ProGrammar::WrappedModule(Host::CandidateTest).candidate(0).file)
          .to match(/helper1/)
      end
      ...
      it(
        ...
        expect(ProGrammar::WrappedModule(Host::CandidateTest).candidate(1).file)
          .to match(/helper2/)
      end

      it 'returns class with third largest number of methods as third ranked candidate' do
        expect(ProGrammar::WrappedModule(Host::CandidateTest).candidate(2).file)
          .to match(/#{__FILE__}/)
      end

      it 'should raise when trying to access non-existent candidate' do
        expect { ProGrammar::WrappedModule(Host::CandidateTest).candidate(3) }
    ...
    describe "source_location" do
      it 'should return primary candidates source_location by default' do
        wm = ProGrammar::WrappedModule(Host::CandidateTest)
      ...
      it 'returns the location of the outer module if an inner module has methods' do
        wm = ProGrammar::WrappedModule(Host::ForeverAlone)
      ...
      it 'should return nil if no source_location can be found' do
        expect(ProGrammar::WrappedModule(Host::PitifullyBlank).source_location).to eq nil
      end
    end
    ...
    describe "source" do
      ...
      it 'should return source for highest ranked candidate' do
        expect(ProGrammar::WrappedModule(Host::CandidateTest).candidate(0).source)
          .to match(/test1/)
      end

      it 'should return source for second ranked candidate' do
        expect(ProGrammar::WrappedModule(Host::CandidateTest).candidate(1).source)
          .to match(/test4/)
      end

      it 'should return source for third ranked candidate' do
        expect(ProGrammar::WrappedModule(Host::CandidateTest).candidate(2).source)
          .to match(/test6/)
      end

      it 'should return source for deeply nested class' do
        expect(ProGrammar::WrappedModule(Host::ForeverAlone::DoublyNested::TriplyNested).source)
          .to match(/nested_method/)
        mod = ProGrammar::WrappedModule(Host::ForeverAlone::DoublyNested::TriplyNested)
    ...
    describe "doc" do
      ...
      it 'should return doc for highest ranked candidate' do
        expect(ProGrammar::WrappedModule(Host::CandidateTest).candidate(0).doc)
          .to match(/rank 0/)
      end

      it 'should return doc for second ranked candidate' do
        expect(ProGrammar::WrappedModule(Host::CandidateTest).candidate(1).doc)
          .to match(/rank 1/)
      end

      it 'should return doc for third ranked candidate' do
        expect(ProGrammar::WrappedModule(Host::CandidateTest).candidate(2).doc)
          .to match(/rank 2/)
      end

      it 'should return docs for deeply nested class' do
        expect(ProGrammar::WrappedModule(Host::ForeverAlone::DoublyNested::TriplyNested).doc)
  ...
  describe ".method_prefix" do
    ...
    it "should return Foo# for normal classes" do
      expect(ProGrammar::WrappedModule.new(Foo).method_prefix).to eq "Foo#"
    end

    it "should return Bar# for modules" do
      expect(ProGrammar::WrappedModule.new(Kernel).method_prefix).to eq "Kernel#"
    end

    it "should return Foo. for singleton classes of classes" do
      expect(ProGrammar::WrappedModule.new(class << Foo; self; end).method_prefix).to eq "Foo."
    end

    example "of singleton classes of objects" do
      expect(ProGrammar::WrappedModule.new(class << @foo; self; end).method_prefix).to eq "self."
    end

    example "of anonymous classes should not be empty" do
      expect(ProGrammar::WrappedModule.new(Class.new).method_prefix).to match(/#<Class:.*>#/)
    end

    example "of singleton classes of anonymous classes should not be empty" do
      expect(ProGrammar::WrappedModule.new(class << Class.new; self; end).method_prefix)
        .to match(/#<Class:.*>./)
    end
  end
  ...
  describe ".singleton_class?" do
    it "should be true for singleton classes" do
      mod = ProGrammar::WrappedModule.new(class << Object.new; self; end)
      expect(mod).to be_singleton_class
    end

    it "should be false for normal classes" do
      expect(ProGrammar::WrappedModule.new(Class.new).singleton_class?).to eq false
    end

    it "should be false for modules" do
      expect(ProGrammar::WrappedModule.new(Module.new).singleton_class?).to eq false
    end
  end

  describe ".singleton_instance" do
    it "should raise an exception when called on a non-singleton-class" do
      expect { ProGrammar::WrappedModule.new(Class).singleton_instance }
    ...
    it "should return the attached object" do
      ...
      expect(ProGrammar::WrappedModule.new(mod).singleton_instance).to eq(instance)

      klass = class << Object; self; end
      expect(ProGrammar::WrappedModule.new(klass).singleton_instance).to equal(Object)
    end
  end
  ...
  describe ".super" do
    describe "receiver is a class" do
      ...
      it 'should return superclass for a wrapped class' do
        expect(ProGrammar::WrappedModule(@c).super.wrapped).to eq @b
      end

      it 'should return nth superclass for a wrapped class' do
        d = Class.new(@c)
        expect(ProGrammar::WrappedModule(d).super(2).wrapped).to eq @b
      end

      it 'should ignore modules when retrieving nth superclass' do
        expect(ProGrammar::WrappedModule(@c).super(2).wrapped).to eq @a
      end

      it 'should return nil when no nth superclass exists' do
        expect(ProGrammar::WrappedModule(@c).super(10)).to eq nil
      end

      it 'should return self when .super(0) is used' do
        c = ProGrammar::WrappedModule(@c)
    ...
    describe "receiver is a module" do
      ...
      it 'should not ignore modules when retrieving supers' do
        expect(ProGrammar::WrappedModule(@m3).super.wrapped).to eq @m2
      end

      it 'should retrieve nth super' do
        expect(ProGrammar::WrappedModule(@m3).super(2).wrapped).to eq @m1
      end

      it 'should return self when .super(0) is used' do
        m = ProGrammar::WrappedModule(@m1)
