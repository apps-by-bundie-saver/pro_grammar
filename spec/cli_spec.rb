...
RSpec.describe ProGrammar::CLI do
  ...
  describe ".add_plugin_options" do
    ...
    it "loads cli options of plugins" do
      ...
      plugins = { 'pro_grammar-testplugin' => plugin_mock }
      expect(ProGrammar).to receive(:plugins).and_return(plugins)
  ...
  describe ".parse_options" do
    context "when option exists" do
      ...
      it "initializes session setup" do
        expect(ProGrammar).to receive(:initial_session_setup)
        described_class.parse_options(%w[-v])
      end

      it "finalizes session setup" do
        expect(ProGrammar).to receive(:final_session_setup)
        described_class.parse_options(%w[-v])
      end
    end
    ...
    context "when option doesn't exist" do
      it "raises error" do
        expect { described_class.parse_options(['--nothing']) }
          .to raise_error(ProGrammar::CLI::NoOptionsError)
      end
    end
  ...
  describe ".start" do
    before do
      # Don't start ProGrammar session in the middle of tests.
      allow(ProGrammar).to receive(:start)

      described_class.options = proc {}
    end

    it "sets ProGrammar.cli to true" do
      opts = described_class.parse_options(%w[])
      described_class.start(opts)
      expect(ProGrammar.cli).to be_truthy
    end
    ...
    context "when the context option is provided" do
      ...
      it "initializes session setup" do
        expect(ProGrammar).to receive(:initial_session_setup).twice
        opts = described_class.parse_options(%w[--context=Object])
        described_class.start(opts)
      end

      it "finalizes session setup" do
        expect(ProGrammar).to receive(:final_session_setup).twice
        opts = described_class.parse_options(%w[--context=Object])
        described_class.start(opts)
      end

      it "starts ProGrammar in the provided context" do
        expect(ProGrammar).to receive(:start).with(
    ...
    context "when the context option is not provided" do
      ...
      it "starts ProGrammar in the top level" do
        expect(ProGrammar).to receive(:start).with(
          instance_of(Binding), input: instance_of(StringIO)
        ) do |binding, _opts|
          expect(binding.eval('self')).to eq(ProGrammar.main)
        end
        opts = described_class.parse_options(%w[])
        described_class.start(opts)
      end
    ...
    context "when there are some input args" do
      ...
      it "loads files through repl and exits" do
        expect(ProGrammar).to receive(:load_file_through_repl).with(match(%r{pro_grammar/foo}))
        expect(Kernel).to receive(:exit)
    ...
    context "when 'pro_grammar' is passed as an input arg" do
      before { described_class.options = proc {} }

      it "does not load files through repl" do
        expect(ProGrammar).not_to receive(:load_file_through_repl)

        opts = described_class.parse_options(%w[pro_grammar])
        described_class.start(opts)
      end
    end
  end
