...
RSpec.describe ProGrammar::ClassCommand do
  ...
  describe "#call" do
    ...
    it "sets command's opts" do
      expect { subject.call }.to change { subject.opts }
        .from(nil).to(an_instance_of(ProGrammar::Slop))
    end
    ...
    context "when help is invoked" do
      ...
      it "returns void value" do
        expect(subject.call('--help')).to eql(ProGrammar::Command::VOID_VALUE)
      end
    end
  ...
  describe "#slop" do
    ...
    it "returns a Slop instance" do
      expect(subject.slop).to be_a(ProGrammar::Slop)
    end
  ...
  describe "#process" do
    it "raises CommandError" do
      expect { subject.process }
        .to raise_error(ProGrammar::CommandError, /not implemented/)
    end
  end
end
