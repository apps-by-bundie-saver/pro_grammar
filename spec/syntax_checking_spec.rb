...
describe ProGrammar do
  ...
  [
    ...
    it "should not raise an error on broken lines: #{foo.join('\\n')}" do
      redirect_pro_grammar_io(InputTester.new(*foo), @str_output) do
        ProGrammar.start
      end
  ...
  [
    ...
    it "should display correct number of errors on invalid syntax like #{foo.inspect}" do
      ...
      pro_grammar_output = mock_pro_grammar(*foo)
      expect(pro_grammar_output.scan(/SyntaxError/).count).to eq(error_count)
    end
  end
  ...
  it "should not intefere with syntax errors explicitly raised" do
    ...
    redirect_pro_grammar_io(input_tester, @str_output) do
      ProGrammar.start
    end
  ...
  it "should allow trailing , to continue the line" do
    expect(ProGrammar::Code.complete_expression?("puts 1, 2,")).to eq false
  end

  it "should complete an expression that contains a line ending with a ," do
    expect(ProGrammar::Code.complete_expression?("puts 1, 2,\n3")).to eq true
  end

  it "should not suppress the error output if the line ends in ;" do
    expect(mock_pro_grammar("raise RuntimeError, 'foo';")).to match(/RuntimeError/)
  end

  it "should not clobber _ex_ on a SyntaxError in the repl" do
    output = mock_pro_grammar(
  ...
  it "should allow whitespace delimeted strings" do
    expect(mock_pro_grammar('"%s" % % foo ')).to match(/"foo"/)
  end
  ...
  it "should allow whitespace delimeted strings ending on the first char of a line" do
    expect(mock_pro_grammar('"%s" % % ', ' #done!')).to match(/"\\n"/)
  end
end
