...
describe ProGrammar::Hooks do
  ...
  describe ".default" do
    ...
    context "when pro_grammar instance is quiet" do
      let(:pro_grammar_instance) { ProGrammar.new(quiet: true) }

      it "doesn't run the whereami command" do
        expect(pro_grammar_instance).not_to receive(:run_command)
        hooks = described_class.default
        hooks.exec_hook(:before_session, StringIO.new, {}, pro_grammar_instance)
      end
    end

    context "when pro_grammar instance is not quiet" do
      let(:pro_grammar_instance) { ProGrammar.new(quiet: false) }
      let(:output) { StringIO.new }

      it "runs the whereami command" do
        expect(pro_grammar_instance).to receive(:run_command).with('whereami --quiet')
        hooks = described_class.default
        hooks.exec_hook(:before_session, StringIO.new, {}, pro_grammar_instance)
      end
    end
  ...
  describe "ProGrammar::Hooks#merge" do
    describe "merge!" do
      it 'should merge in the ProGrammar::Hooks' do
        h1 = ProGrammar::Hooks.new.add_hook(:test_hook, :testing) {}
        h2 = ProGrammar::Hooks.new
      ...
      it 'should NOT overwrite hooks belonging to shared event in receiver' do
        h1 = ProGrammar::Hooks.new.add_hook(:test_hook, :testing) {}
        callable = proc {}
        h2 = ProGrammar::Hooks.new.add_hook(:test_hook, :testing2, callable)
      ...
      it 'should overwrite identical hook in receiver' do
        callable1 = proc { :one }
        h1 = ProGrammar::Hooks.new.add_hook(:test_hook, :testing, callable1)
        callable2 = proc { :two }
        h2 = ProGrammar::Hooks.new.add_hook(:test_hook, :testing, callable2)
      ...
      describe "merge" do
        ...
        it 'should contain hooks from original instance' do
          h1 = ProGrammar::Hooks.new.add_hook(:test_hook, :testing) {}
          h2 = ProGrammar::Hooks.new.add_hook(:test_hook2, :testing) {}
  ...
  describe "dupping a ProGrammar::Hooks instance" do
  ...
  describe "integration tests" do
    describe "when_started hook" do
      it 'should yield options to the hook' do
        options = nil
        ProGrammar.config.hooks.add_hook(:when_started, :test_hook) do |_target, opt, _|
          options = opt
        end

        redirect_pro_grammar_io(StringIO.new("exit"), StringIO.new) do
          ProGrammar.start binding, hello: :baby
        end

        expect(options[:hello]).to eq :baby

        ProGrammar.config.hooks.delete_hook(:when_started, :test_hook)
      end

      describe "target" do
        it 'should yield the target, as a binding ' do
          b = nil
          ProGrammar.config.hooks.add_hook(:when_started, :test_hook) do |target, _opt, _|
            b = target
          end

          redirect_pro_grammar_io(StringIO.new("exit"), StringIO.new) do
            ProGrammar.start 5, hello: :baby
          end
      ...
      it 'should allow overriding of target (and binding_stack)' do
        ...
        ProGrammar.config.hooks.add_hook(
          :when_started, :test_hook
        ) do |_target, _opt, pro_grammar_instance|
          pro_grammar_instance.binding_stack = [ProGrammar.binding_for(o)]
        end

        redirect_pro_grammar_io(InputTester.new("@value = true", "exit-all")) do
          ProGrammar.start binding, hello: :baby
        end
    ...
    describe "after_session hook" do
      it 'should always run, even if uncaught exception bubbles out of repl' do
        ...
        old_ew = ProGrammar.config.unrescued_exceptions
        ProGrammar.config.unrescued_exceptions << o.great_escape

        array = [1, 2, 3, 4, 5]

        begin
          redirect_pro_grammar_io(StringIO.new("raise great_escape"), StringIO.new) do
            ProGrammar.start(
              o,
              hooks: ProGrammar::Hooks.new.add_hook(:after_session, :cleanup) { array = nil }
            )
          end
        ...
        # cleanup after test
        ProGrammar.config.unrescued_exceptions = old_ew
      end

      describe "before_eval hook" do
        describe "modifying input code" do
          it 'should replace input code with code determined by hook' do
            hooks = ProGrammar::Hooks.new.add_hook(:before_eval, :quirk) do |code, _pro_grammar|
              code.replace(":little_duck")
            end
            redirect_pro_grammar_io(InputTester.new(":jemima", "exit-all"), out = StringIO.new) do
              ProGrammar.start(self, hooks: hooks)
            end
          ...
          it 'should not interfere with command processing when replacing input code' do
            commands = ProGrammar::CommandSet.new do
              import_from ProGrammar::Commands, "exit-all"
            ...
            redirect_pro_grammar_io(
              ...
              ProGrammar.start(self, hooks: hooks, commands: commands)
            end
      ...
      describe "exceptions" do
        before do
          ProGrammar.config.hooks.add_hook(:after_eval, :baddums) { raise "Baddums" }
          ProGrammar.config.hooks.add_hook(:after_eval, :simbads) { raise "Simbads" }
        end

        after do
          ProGrammar.config.hooks.delete_hook(:after_eval, :baddums)
          ProGrammar.config.hooks.delete_hook(:after_eval, :simbads)
        end
        it "should not raise exceptions" do
          expect { mock_pro_grammar("1", "2", "3") }.to_not raise_error
        end

        it "should print out a notice for each exception raised" do
          expect(mock_pro_grammar("1")).to match(
