...
describe ProGrammar::Prompt do
  ...
  describe "prompt invocation" do
    let(:pro_grammar) { ProGrammar.new }
    ...
    it "computes prompt name dynamically" do
      ...
      pro_grammar.config.prompt_name = ProGrammar::Config::LazyValue.new { enum.next }
      expect(proc.call(Object.new, 1, pro_grammar, '>')).to eq('[1] a(#<Object>):1> ')
      expect(proc.call(Object.new, 1, pro_grammar, '>')).to eq('[1] b(#<Object>):1> ')
    end
  end
end
