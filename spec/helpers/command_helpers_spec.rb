...
RSpec.describe ProGrammar::Helpers::CommandHelpers do
  ...
  describe "#internal_binding?" do
    ...
    context "when target's __method__ returns __pro_grammar__" do
      it "returns true" do
    ...
    context "when there's name but no corresponding method" do
      it "raises MethodNotFound" do
        expect { subject.get_method_or_raise('foobar', binding) }
          .to raise_error(ProGrammar::MethodNotFound, /method.+could not be found/)
      end
    end
    ...
    context "when super opt is provided but there's no super method" do
      ...
      it "raises MethodNotFound" do
        expect { subject.get_method_or_raise('test_method', test_binding, super: 1) }
          .to raise_error(ProGrammar::MethodNotFound, /has no super method/)
      end
    end
    ...
    context "when there's no method name" do
      it "raises MethodNotFound" do
        expect { subject.get_method_or_raise(nil, binding) }
          .to raise_error(ProGrammar::MethodNotFound, /no method name given/)
      end
    end
  end
  ...
  describe "#set_file_and_dir_locals" do
    ...
    it "injects local variable _file_" do
      subject.set_file_and_dir_locals('foo/test.rb', pro_grammar_instance, context)
    ...
    it "sets pro_grammar instance's last_file to _file_" do
      subject.set_file_and_dir_locals('foo/test.rb', pro_grammar_instance, context)
      expect(pro_grammar_instance.last_file).to end_with('test.rb')
    end

    it "sets pro_grammar instance's last_dir to _dir_" do
      subject.set_file_and_dir_locals('foo/test.rb', pro_grammar_instance, context)
      expect(pro_grammar_instance.last_dir).to end_with('foo')
    end
  end
end
