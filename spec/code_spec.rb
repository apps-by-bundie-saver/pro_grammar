...
RSpec.describe ProGrammar::Code do
  describe "ProGrammar::Code()" do
    ...
    context "when given a Method" do
      ...
      it "reads lines from bound method" do
        expect(ProGrammar::Code(method(:bound_method)).to_s).to eq(
          "def bound_method\n  :test\nend\n"
        )
      end
    end
    ...
    context "when given an UnboundMethod" do
      ...
      it "reads lines from unbound methods" do
        ...
        expect(ProGrammar::Code(unbound_method).to_s).to eq(
          "def unbound_method\n  :test\nend\n"
        )
      end
    end
    ...
    context "when given a Proc" do
      it "reads lines from proc" do
        proc = proc { :proc }
        expect(ProGrammar::Code(proc).to_s).to eq("proc = proc { :proc }\n")
      end
    end

    context "when given a ProGrammar::Method" do
      def bound_method
        :test
      end

      it "reads lines from ProGrammar::Method" do
        method = ProGrammar::Method(method(:bound_method))
        expect(ProGrammar::Code(method).to_s).to eq("def bound_method\n  :test\nend\n")
      end
    end

    context "when given an Array" do
      it "reads lines from the array" do
        expect(ProGrammar::Code(%w[1 2 3]).length).to eq(3)
      end
    end
  end
  ...
  describe ".from_file" do
    ...
    it "raises error when file doesn't exist" do
      expect { ProGrammar::Code.from_file('abcd') }
    ...
    context "when readling lines from ProGrammar's line buffer" do
      it "reads entered lines" do
        pro_grammar_eval ':hello'
        expect(described_class.from_file('(pro_grammar)').to_s).to eq(":hello\n")
      end

      it "can specify file type manually" do
        expect(described_class.from_file('(pro_grammar)', :c).code_type).to eq(:c)
      end
    end
  ...
  describe ".from_method" do
    it "reads lines from a method's definition" do
      method = ProGrammar::Method.from_obj(described_class, :from_method)
