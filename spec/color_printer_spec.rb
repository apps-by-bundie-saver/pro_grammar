...
RSpec.describe ProGrammar::ColorPrinter do
  ...
  describe ".default" do
    ...
    it "prints output prefix with value" do
      described_class.default(StringIO.new, 'foo', pro_grammar_instance)
  ...
  describe ".pp" do
    ...
    context "when #inspect returns an object literal" do
      ...
      context "and when SyntaxHighlighter returns a token starting with '\e'" do
        before do
          expect(ProGrammar::SyntaxHighlighter).to receive(:keyword_token_color)
            .and_return("\e[32m")
        end
    ...
    context "when #inspect raises ProGrammar::Pager::StopPaging" do
      let(:klass) do
        Class.new do
          def inspect
            raise ProGrammar::Pager::StopPaging
          end
        end
      end
      ...
      it "propagates the error" do
        expect { described_class.pp(klass.new, output) }
          .to raise_error(ProGrammar::Pager::StopPaging)
      end
    end
  end
end
