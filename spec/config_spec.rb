...
RSpec.describe ProGrammar::Config do
  specify { expect(subject.input).to respond_to(:readline) }
  specify { expect(subject.output).to be_an(IO) }
  specify { expect(subject.commands).to be_a(ProGrammar::CommandSet) }
  specify { expect(subject.prompt_name).to be_a(String) }
  specify { expect(subject.prompt).to be_a(ProGrammar::Prompt) }
  ...
  specify { expect(subject.hooks).to be_a(ProGrammar::Hooks) }
  ...
  specify { expect(subject.completer).to eq(ProGrammar::InputCompleter) }
  specify { expect(subject.history).to be_a(ProGrammar::History) }
  ...
  describe "#rc_file" do
    context "when $PRYRC env variable is set" do
      before do
        ...
        allow(ProGrammar::Env).to receive(:[]).with('PRYRC').and_return('/foo/pro_grammarrc')
      end

      it "defaults to the value of PRYRC env variable" do
        expect(subject.rc_file).to eq('/foo/pro_grammarrc')
      end
    end

    context "when ~/.pro_grammarrc exists and $XDG_CONFIG_HOME is undefined" do
      before do
        allow(File).to receive(:exist?)
        expect(File).to receive(:exist?)
          .with(File.expand_path('~/.pro_grammarrc')).and_return(true)
      ...
      it "defaults to ~/.pro_grammarrc" do
        expect(subject.rc_file).to eq('~/.pro_grammarrc')
      end
    end
    ...
    context "when $XDG_CONFIG_HOME is defined" do
      ...
      context "and when ~/.pro_grammarrc exists" do
        before do
          allow(File).to receive(:exist?)
            .with(File.expand_path('~/.pro_grammarrc')).and_return(true)
        end

        it "defaults to $XDG_CONFIG_HOME/pro_grammar/pro_grammarrc" do
          expect(subject.rc_file).to eq('/xdg_home/pro_grammar/pro_grammarrc')
        end
      end

      context "and when ~/.pro_grammarrc doesn't exist" do
        before do
          allow(File).to receive(:exist?)
            .with(File.expand_path('~/.pro_grammarrc')).and_return(false)
        end
  ...
  describe "#method_missing" do
    ...
    context "when invoked method is a LazyValue" do
      it "defines a callable attribute" do
        subject.foo = ProGrammar::Config::LazyValue.new { 1 }
        expect(subject.foo).to eq(1)
      end
    end
  end
  ...
  describe "#control_d_handler=" do
    context "when the handler expects multiple arguments" do
      it "prints a warning" do
        expect(ProGrammar::Warning).to receive(:warn).with(
    ...
    context "when the handler expects just one argument" do
      it "doesn't print a warning" do
        expect(ProGrammar::Warning).not_to receive(:warn)
  ...
  describe "#control_d_handler" do
    ...
    context "when it returns a callable accepting one argument" do
      context "and when it is called with one argument" do
        it "calls the handler with a pro_grammar instance" do
          subject.control_d_handler = proc do |arg|
            expect(arg).to eql(pro_grammar_instance)
          end
          subject.control_d_handler.call(pro_grammar_instance)
        end
      end

      context "and when it is called with multiple arguments" do
        before { allow(ProGrammar::Warning).to receive(:warn) }
    ...
    context "when it returns a callabale with two arguments" do
      ...
      context "and when it's called with one argument" do
        it "calls the handler with a eval_string and a pro_grammar instance" do
          subject.control_d_handler = proc do |arg1, arg2|
            expect(arg1).to eq('')
            expect(arg2).to eql(pro_grammar_instance)
          end
          subject.control_d_handler.call(pro_grammar_instance)
        end
      end
