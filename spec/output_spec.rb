...
RSpec.describe ProGrammar::Output do
  let(:output) { StringIO.new }
  let(:pro_grammar_instance) { ProGrammar.new(output: output) }

  subject { described_class.new(pro_grammar_instance) }
  ...
  describe "#puts" do
    ...
    context "when the given pro_grammar instance has 'color' enabled" do
      let(:pro_grammar_instance) { ProGrammar.new(output: output, color: true) }
    ...
    context "when the given pro_grammar instance has 'color' disabled" do
      let(:pro_grammar_instance) { ProGrammar.new(output: output, color: false) }
  ...
  describe "#size" do
    context "when the output is a tty and responds to winsize" do
      before do
        skip("io/console doesn't support JRuby") if ProGrammar::Helpers::Platform.jruby?
    ...
    context "when the output is not a tty" do
      ...
      context "and ENV has size info in ROWS and COLUMNS" do
        before do
          expect(ProGrammar::Env).to receive(:[]).with('ROWS').and_return(2)
          expect(ProGrammar::Env).to receive(:[]).with('COLUMNS').and_return(2)
        end
      ...
      context "and ENV has size info in LINES and COLUMNS" do
        before do
          expect(ProGrammar::Env).to receive(:[]).with('LINES').and_return(3)
          expect(ProGrammar::Env).to receive(:[]).with('COLUMNS').and_return(2)
        end
      ...
      context "and when there's ANSICON ENV variable" do
        context "and when it can be matched" do
          context "and when the size consists of positive integers" do
            before do
              expect(ProGrammar::Env).to receive(:[]).with('ANSICON').and_return('(5x5)')
            end
          ...
          context "and when the size has a zero column" do
            before do
              expect(ProGrammar::Env).to receive(:[]).with('ANSICON').and_return('(0x0)')
            end
