...
RSpec.describe ProGrammar::ExceptionHandler do
  describe ".handle_exception" do
    ...
    context "when exception is a UserError and a SyntaxError" do
      let(:exception) do
        SyntaxError.new('cool syntax error, dude').extend(ProGrammar::UserError)
      end

      it "prints the syntax error with customized message" do
        described_class.handle_exception(output, exception, pro_grammar_instance)
    ...
    context "when exception is a standard error" do
      let(:exception) do
        ...
        error.set_backtrace(["/bin/pro_grammar:23:in `<main>'"])
        error
      end
      ...
      it "prints standard error message" do
        ...
        expect(output.string)
          .to eq("StandardError: oops\nfrom /bin/pro_grammar:23:in `<main>'\n")
      end
    end
