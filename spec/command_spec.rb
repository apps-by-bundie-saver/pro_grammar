...
RSpec.describe ProGrammar::Command do
  subject do
  ...
  describe ".name" do
    it "returns the name of the command" do
      expect(subject.name).to eq('#<class(ProGrammar::Command nil)>')
    end
  ...
  describe ".group" do
    ...
    context "when source file matches a pro_grammar command" do
      before do
        expect_any_instance_of(ProGrammar::Method).to receive(:source_file)
          .and_return('/pro_grammar/test_commands/test_command.rb')
      end
    ...
    context "when source file matches a pro_grammar plugin" do
      before do
        expect_any_instance_of(ProGrammar::Method).to receive(:source_file)
          .and_return('pro_grammar-test-1.2.3')
      end

      it "sets group name to plugin name" do
        expect(subject.group).to eq('pro_grammar-test (v1.2.3)')
      end
    end

    context "when source file matches 'pro_grammarrc'" do
      before do
        expect_any_instance_of(ProGrammar::Method).to receive(:source_file)
          .and_return('pro_grammarrc')
      end

      it "sets group name to pro_grammarrc" do
        expect(subject.group).to eq('pro_grammarrc')
      end
    end
  ...
  describe "#run" do
    ...
    subject do
      ...
      command.new(command_set: command_set, pro_grammar_instance: ProGrammar.new)
    end
  ...
  describe "#void" do
    it "returns void value" do
      expect(subject.new.void).to eq(ProGrammar::Command::VOID_VALUE)
    end
  end
  ...
  describe "#state" do
    ...
    subject { Class.new(described_class).new(pro_grammar_instance: ProGrammar.new) }
    ...
    context "when given string doesn't match a command" do
      it "raises CommandError" do
        expect { subject.tokenize('boom') }
          .to raise_error(ProGrammar::CommandError, /command which didn't match/)
      end
    end
  ...
  describe "#process_line" do
    ...
    describe "collision warnings" do
      context "when collision warnings are configured" do
        before do
          expect(ProGrammar.config).to receive(:collision_warning).and_return(true)
        end
      ...
      context "when collision warnings are not set" do
        before do
          expect(ProGrammar.config).to receive(:collision_warning).and_return(false)
        end
