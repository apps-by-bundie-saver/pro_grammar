...
RSpec.describe ProGrammar::CommandSet do
  let(:set) do
  ...
  describe "#[]=" do
    ...
    context "when given command is not a subclass of ProGrammar::Command" do
      it "raises TypeError" do
        expect { subject['test'] = 1 }
          .to raise_error(TypeError, 'command is not a subclass of ProGrammar::Command')
      end
    end
  end
  ...
  describe "#add_command" do
    it "adds a command" do
      subject.add_command(Class.new(ProGrammar::Command))
      expect(subject.count).to eq(1)
    end
  end
