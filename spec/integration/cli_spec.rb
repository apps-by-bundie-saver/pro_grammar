...
RSpec.describe 'The bin/pro_grammar CLI' do
  let(:ruby) { RbConfig.ruby.shellescape }
  let(:pro_grammar_dir) { File.expand_path(File.join(__FILE__, '../../../lib')).shellescape }
  let(:clean_output) do
    # ProGrammar will emit silent garbage because of our auto indent feature.
  ...
  context "ARGV forwarding" do
    ...
    it "forwards ARGV as an empty array when - is passed without following arguments" do
      out = clean_output.call(`#{ruby} -I#{pro_grammar_dir} bin/pro_grammar -e #{code} -`)
    ...
    it "forwards its remaining arguments as ARGV when - is passed" do
      out = clean_output.call(
        `#{ruby} -I#{pro_grammar_dir} bin/pro_grammar -e #{code} - 1 -foo --bar --baz daz`
      )
  ...
  context '-I path' do
    it 'adds an additional path to $LOAD_PATH' do
      ...
      out = clean_output.call(
        `#{ruby} -I#{pro_grammar_dir} bin/pro_grammar -I /added/at/cli -e '#{code}'`
      )
    ...
    it 'adds multiple additional paths to $LOAD_PATH' do
      ...
      out = clean_output.call(
        ...
        `#{ruby} -I#{pro_grammar_dir} bin/pro_grammar -I /added-1/at/cli -I /added/at/cli/also -e '#{code}'`
