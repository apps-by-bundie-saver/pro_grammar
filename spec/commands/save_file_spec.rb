...
describe "save-file" do
  before do
    @tf = Tempfile.new(["pro_grammar", ".py"])
    @path = @tf.path
  ...
  describe "saving methods" do
    before do
      ...
      @t = pro_grammar_tester(@o)
    end

    describe "single method" do
      it 'should save a method to a file' do
        @t.eval "save-file --to '#{@path}' baby"
        expect(File.read(@path)).to eq(ProGrammar::Method.from_obj(@o, :baby).source)
      end
      ...
      it 'should save a method to a file truncated by --lines' do
        ...
        expect(File.read(@path)).to eq(
          ProGrammar::Method.from_obj(@o, :baby).source.lines.to_a[1..5].join
        )
      end
    end
    ...
    #     File.read(@path).should == ProGrammar::Method.from_obj(@o, :baby).source +
    #       ProGrammar::Method.from_obj(@o, :bang).source
    #   end
    ...
    #     # must add 1 as first line of method is 1
    #     File.read(@path).should == (ProGrammar::Method.from_obj(@o, :baby).source +
    #       ProGrammar::Method.from_obj(@o, :bang).source).lines.to_a[1..-2].join
    #   end
    ...
    #     # must add 1 as first line of method is 1
    #     File.read(@path).should == (ProGrammar::Method.from_obj(@o, :baby).source +
    #       ProGrammar::Method.from_obj(@o, :bang).source).lines.to_a[0]
    #   end
    # end
  end
  ...
  describe "saving commands" do
    it 'should save a command to a file' do
      ...
      cmd_source = ProGrammar.config.commands["show-source"].source
  ...
  #       ProGrammar::Method.from_obj(@o, :baby).source
  #   end
