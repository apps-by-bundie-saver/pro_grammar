...
describe 'cd' do
  before do
    ...
    @t = pro_grammar_tester(@o) do
      def mapped_binding_stack
        binding_stack.map { |b| b.eval('self') }
      end

      def binding_stack
        pro_grammar.binding_stack.dup
      end

      def command_state
        pro_grammar.commands['cd'].state
      end

      def old_stack
        pro_grammar.commands['cd'].state.old_stack.dup
      end
    end
  end

  after { ProGrammar::CommandState.default.reset('cd') }

  describe 'old stack toggling with `cd -`' do
    describe 'in fresh pro_grammar instance' do
    ...
    describe 'when an error was raised' do
      it 'should not toggle and should keep correct stacks' do
        expect { @t.eval 'cd %' }.to raise_error ProGrammar::CommandError
    ...
    describe 'when using ^D (Control-D) key press' do
      it 'should keep correct old binding' do
        @t.eval 'cd :john_dogg', 'cd :mon_dogg', 'cd :kyr_dogg',
                'ProGrammar.config.control_d_handler.call(pro_grammar_instance)'
  ...
  it 'should not cd into complex input when it encounters an exception' do
    expect { @t.eval 'cd 1/2/swoop_a_doop/3' }.to raise_error ProGrammar::CommandError
  ...
  # https://github.com/pro_grammar/pro_grammar/issues/1596
