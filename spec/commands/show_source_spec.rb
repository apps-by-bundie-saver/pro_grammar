...
describe "show-source" do # rubocop:disable Metrics/BlockLength
  ...
  it "should output a method's source" do
    expect(pro_grammar_eval(binding, 'show-source @o.sample_method')).to match(/def @o.sample/)
  end

  it "should output help" do
    expect(pro_grammar_eval('show-source -h')).to match(/Usage:\s+show-source/)
  end

  it "should output a method's source with line numbers" do
    expect(pro_grammar_eval(binding, 'show-source -l @o.sample_method'))
      .to match(/\d+: def @o.sample/)
  end

  it "should output a method's source with line numbers starting at 1" do
    expect(pro_grammar_eval(binding, 'show-source -b @o.sample_method'))
      .to match(/1: def @o.sample/)
  end

  it "should output a method's source if inside method and no name given" do
    def @o.sample
      pro_grammar_eval(binding, 'show-source')
    end
    docs = @o.sample
  ...
  it "should output a method's source inside method using the -l switch" do
    def @o.sample
      pro_grammar_eval(binding, 'show-source -l')
    end
    docs = @o.sample
    expect(docs).to match(/def @o.sample/)
  end
  ...
  it "should find methods even if there are spaces in the arguments" do
    ...
    out = pro_grammar_eval(binding, "show-source @o.foo('bar', 'baz bam').foo")
  ...
  it "should find methods even if the object overrides method method" do
    ...
    expect(pro_grammar_eval(binding, "show-source _c.new.method")).to match(/98/)
  end
  ...
  it "should not show the source when a non-extant method is requested" do
    ...
    expect(mock_pro_grammar(binding, "show-source _c#wrongmethod"))
  ...
  it "doesn't show the source and deliver an error message without exclamation point" do
    ...
    expect(mock_pro_grammar(binding, "show-source _c#wrongmethod")).to eq(error_message)
  end
  ...
  it "should find instance_methods if the class overrides instance_method" do
    ...
    expect(pro_grammar_eval(binding, "show-source _c#method")).to match(/98/)
  end
  ...
  it "should find instance methods with self#moo" do
    ...
    expect(pro_grammar_eval(binding, "cd _c", "show-source self#moo")).to match(/ve over/)
  end
  ...
  it "should not find instance methods with self.moo" do
    ...
    expect { pro_grammar_eval(binding, 'cd _c', 'show-source self.moo') }
      .to raise_error(ProGrammar::CommandError, /Couldn't locate/)
  end
  ...
  it "should find normal methods (i.e non-instance methods) by default" do
    ...
    expect(pro_grammar_eval(binding, "cd _c", "show-source moo")).to match(/ve over/)
  end
  ...
  it "should raise a CommandError when super method doesn't exist" do
    ...
    expect { pro_grammar_eval(binding, "show-source --super @o.foo") }
      .to raise_error(ProGrammar::CommandError, /No superclass found/)
  end

  it "should output the source of a method defined inside ProGrammar" do
    out = pro_grammar_eval("def dyn_method\n:test\nend", 'show-source dyn_method')
    expect(out).to match(/def dyn_method/)
    Object.remove_method :dyn_method
  end

  it 'should output source for an instance method defined inside pro_grammar' do
  ...
  it "should output the source of a command defined inside ProGrammar" do
    command_definition = %(
      ProGrammar.config.commands.command "hubba-hubba" do
        puts "that's what she said!"
      end
    )
    out = pro_grammar_eval(command_definition, 'show-source hubba-hubba')
    expect(out).to match(/what she said/)
    ProGrammar.config.commands.delete "hubba-hubba"
  end
  ...
  context "when there's no source code but the comment exists" do
    before do
      ...
      allow_any_instance_of(ProGrammar::Method).to receive(:source).and_return(nil)
    end
    ...
    it "outputs zero line numbers" do
      out = pro_grammar_eval('show-source Foo#bar')
      expect(out).to match(/
        Owner:\sFoo
        .+
  ...
  describe "finding super methods with help of `--super` switch" do
    ...
    it "finds super methods with explicit method argument" do
      ...
      expect(pro_grammar_eval(binding, "show-source --super o.foo")).to match(/:super_wibble/)
    end
    ...
    it "finds super methods without explicit method argument" do
      ...
      def o.foo(*bars)
        @foo = :wibble
        pro_grammar_eval(binding, 'show-source --super')
      end
    ...
    it "finds super methods with multiple --super " do
      ...
      def o.foo(*bars)
        @foo = :wibble
        pro_grammar_eval(binding, 'show-source --super --super')
      end
  ...
  describe "on sourcable objects" do
    it "should output source defined inside pro_grammar" do
      pro_grammar_tester.tap do |t|
    ...
    it "should output source for procs/lambdas stored in variables" do
      ...
      expect(pro_grammar_eval(binding, 'show-source _hello')).to match(/proc \{ puts/)
    end

    it "should output source for procs/lambdas stored in constants" do
      HELLO = proc { puts 'hello world!' }
      expect(pro_grammar_eval(binding, "show-source HELLO")).to match(/proc \{ puts/)
    ...
    it "should output source for method objects" do
      ...
      _meth = @o.method(:hi)
      expect(pro_grammar_eval(binding, "show-source _meth")).to match(/puts 'hi world'/)
    end

    describe "on variables that shadow methods" do
      before do
        @t = pro_grammar_tester.eval(unindent(<<-SHADOWED_VAR))
          class ::TestHost
            def hello
              hello = proc { ' smile ' }
              _foo = hello
              pro_grammar_tester(binding)
            end
          end
          ::TestHost.new.hello
        SHADOWED_VAR
  ...
  describe "on variable or constant" do
    ...
    it "outputs source of its class if variable doesn't respond to source_location" do
      ...
      expect(pro_grammar_eval(binding, 'show-source _test_host'))
    ...
    it "outputs source of its class if constant doesn't respond to source_location" do
      ...
      expect(pro_grammar_eval(binding, 'show-source TEST_HOST'))
        .to match(/class TestHost\n.*def hello/)
  ...
  describe "on modules" do
    ...
    describe "basic functionality, should find top-level module definitions" do
      ...
      it 'should show source for a super class when Const = Class.new syntax is used' do
        expect(pro_grammar_eval('show-source -s ShowSourceTestClassWeirdSyntax'))
          .to match(/class Object/)
      end

      it 'should show source for a module when Const = Module.new syntax is used' do
        expect(pro_grammar_eval('show-source ShowSourceTestModuleWeirdSyntax'))
          .to match(/ShowSourceTestModuleWeirdSyntax = Module.new/)
      end
    end

    before do
      pro_grammar_eval(unindent(<<-CLASSES))
        class Dog
          def woof
          end
        end
    ...
    describe "in REPL" do
      it 'should find class defined in repl' do
        expect(pro_grammar_eval('show-source TobinaMyDog')).to match(/class TobinaMyDog/)
      end

      it 'should find superclass defined in repl' do
        expect(pro_grammar_eval('show-source -s TobinaMyDog')).to match(/class Dog/)
      end
    end
    ...
    it 'should lookup module name with respect to current context' do
      temporary_constants(:AlphaClass, :BetaClass) do
        ...
        expect(pro_grammar_eval(AlphaClass, 'show-source BetaClass')).to match(/def beta/)
      end
    end
    ...
    it 'should lookup nested modules' do
      temporary_constants(:AlphaClass) do
        ...
        expect(pro_grammar_eval('show-source AlphaClass::BetaClass')).to match(/class Beta/)
      end
    end

    # note that pro_grammar assumes a class is only monkey-patched at most
    # ONCE per file, so will not find multiple monkeypatches in the
    # SAME file.
    describe "show-source -a" do
      let(:tempfile) { Tempfile.new(%w[pro_grammar .rb]) }
      ...
      context "when there are instance method monkeypatches in different files" do
        ...
        it "shows the source for all monkeypatches" do
          result = pro_grammar_eval('show-source TestClass -a')
      ...
      context "when there are class-eval monkeypatches in different files" do
        ...
        it "ignores -a because object is not a module" do
          result = pro_grammar_eval('show-source TestClass#class_eval_method -a')
      ...
      context "when -a is not used and there are multiple monkeypatches" do
        ...
        it "mentions available monkeypatches" do
          result = pro_grammar_eval('show-source TestClass')
    ...
    describe "when show-source is invoked without a method or class argument" do
      ...
      describe "inside a module" do
        it 'should display module source by default' do
          out = pro_grammar_eval(TestHost::M, 'show-source')
        ...
        it 'should be unable to find module source if no methods defined' do
          expect { pro_grammar_eval(TestHost::C, 'show-source') }
            .to raise_error(ProGrammar::CommandError, /Couldn't locate/)
        end

        it(
          'displays method code (rather than class) if ProGrammar started inside ' \
          'method binding'
        ) do
        ...
        it 'should display class source when inside instance' do
          out = pro_grammar_eval(TestHost::M.new, 'show-source')
          expect(out).to match(/class M/)
          expect(out).to match(/def alpha/)
          expect(out).to match(/def beta/)
        end
        ...
        it 'should allow options to be passed' do
          out = pro_grammar_eval(TestHost::M, 'show-source -b')
        ...
        describe "should skip over broken modules" do
          ...
          it 'should return source for first valid module' do
            out = pro_grammar_eval('show-source BabyDuck::Muesli')
        ...
        describe "monkey-patched C modules" do
          ...
          describe "when current context is a C object" do
            it "should display a warning, and not monkey-patched definition" do
              out = pro_grammar_eval([1, 2, 3], 'show-source')
              expect(out).not_to match(/doge/)
              expect(out).to match(/ProGrammar cannot display the information/)
            end

            it "recommends to use the --all switch when other candidates are found" do
              out = pro_grammar_eval([], 'show-source')
          ...
          describe "when current context is something other than a C object" do
            it "should display a candidate, not a warning" do
              out = pro_grammar_eval('show-source Array')
  ...
  describe "on commands" do
    let(:default_commands) { ProGrammar.config.commands }

    let(:command_set) do
      ProGrammar::CommandSet.new { import ProGrammar::Commands }
    end

    before { ProGrammar.config.commands = command_set }
    after { ProGrammar.config.commands = default_commands }

    describe "block commands" do
      it 'should show source for an ordinary command' do
        command_set.command('foo', :body_of_foo) {}
        expect(pro_grammar_eval(binding, 'show-source foo')).to match(/:body_of_foo/)
      end

      it "should output source of commands using special characters" do
        command_set.command('!%$', 'I gots the yellow fever') {}
        expect(pro_grammar_eval(binding, 'show-source !%$')).to match(/yellow fever/)
      end

      it 'should show source for a command with spaces in its name' do
        command_set.command('foo bar', :body_of_foo_bar) {}
        expect(pro_grammar_eval(binding, 'show-source foo bar')).to match(/:body_of_foo_bar/)
      end

      it 'should show source for a command by listing name' do
        command_set.command(/foo(.*)/, :body_of_foo_bar_regex, listing: "bar") {}
        expect(pro_grammar_eval(binding, 'show-source bar')).to match(/:body_of_foo_bar_regex/)
      end
    end
    ...
    describe "create_command commands" do
      ...
      it 'should show source for a command defined inside pro_grammar' do
        pro_grammar_eval %{
          pro_grammar_instance.commands.create_command "foo", "babble" do
    ...
    describe "real class-based commands" do
      before do
        ...
        class ::TemporaryCommand < ProGrammar::ClassCommand
          match 'temp-command'
          def process
            :body_of_temp
          end
        ...
        ProGrammar.config.commands.add_command(::TemporaryCommand)
      end
      ...
      it 'should show source for a command' do
        expect(pro_grammar_eval('show-source temp-command')).to match(/:body_of_temp/)
      end

      it 'should show source for a command defined inside pro_grammar' do
        pro_grammar_eval %{
          class ::TemporaryCommandInProGrammar < ProGrammar::ClassCommand
            match 'temp-command-in-pro_grammar'
            def process() :body_of_temp end
          end
        }
        ProGrammar.config.commands.add_command(::TemporaryCommandInProGrammar)
        expect(pro_grammar_eval('show-source temp-command-in-pro_grammar')).to match(/:body_of_temp/)
        Object.remove_const(:TemporaryCommandInProGrammar)
      end
    end
  end
  ...
  describe "should set _file_ and _dir_" do
    ...
    it 'should set _file_ and _dir_ to file containing method source' do
      ...
      expect(t.pro_grammar.last_dir).to match(path)

      expect(t.pro_grammar.last_file).to match(tempfile.path)
    end
  end
  ...
  describe "show-source --doc" do
    context "when given a class with a doc" do
      ...
      it "shows documentation for the code object along with source code" do
        expect(pro_grammar_eval(binding, "show-source Foo -d")).to match(
          /Foo has docs\.\n\s+class Foo/
        )
      end
    end
    ...
    context "when given a module with a doc" do
      ...
      it "shows documentation for the code object along with source code" do
        expect(pro_grammar_eval(binding, "show-source TestMod -d")).to match(
          /TestMod has docs\n\s+module TestMod/
        )
      end
    end
    ...
    context "when the Const = Class.new syntax is used" do
      ...
      it "shows documentation for the class" do
        expect(pro_grammar_eval(binding, "show-source TestClass -d")).to match(
    ...
    context "when the current context is a non-nested class" do
      ...
      it "shows docs for the nested classes" do
        expect(pro_grammar_eval(AlphaClass, "show-source -d BetaClass"))
    ...
    context "when given a nested class" do
      ...
      it "shows docs for the nested classes" do
        expect(pro_grammar_eval(AlphaClass, "show-source -d AlphaClass::BetaClass"))
          .to match(/nested beta/)
      end
    end
    ...
    context "when given a method with a doc" do
      ...
      it "finds the method's documentation" do
        expect(pro_grammar_eval(binding, "show-source -d @obj.test_method"))
          .to match(/test doc/)
      end
    end
    ...
    context "when no docs can be found for the given class" do
      ...
      it "raises ProGrammar::CommandError" do
        expect { pro_grammar_eval(binding, "show-source -d TestClass") }
          .to raise_error(ProGrammar::CommandError)
      end
    end
    ...
    context "when the --line-numbers switch is provided" do
      ...
      it "outputs a method's docs with line numbers" do
        expect(pro_grammar_eval(binding, "show-source -d --line-numbers @obj.test_method"))
          .to match(/\d: test doc/)
      end
    end
    ...
    context "when the --base-one switch is provided" do
      ...
      it "outputs a method's docs with line numbering starting at 1" do
        expect(pro_grammar_eval(binding, "show-source -d --base-one @obj.test_method"))
          .to match(/1: test doc/)
      end
    end
    ...
    context "when the current context is a method" do
      it "outputs the method without needing to use its name" do
        ...
        # test method
        def obj.test_method
          pro_grammar_eval(binding, 'show-source -d')
        end
    ...
    context "when given a proc" do
      it "should show documentation for object" do
        ...
        expect(mock_pro_grammar(binding, "show-source -d _the_proc"))
    ...
    context "when no class/module arg is given" do
      ...
      it "returns the doc for the current module" do
        expect(pro_grammar_eval(TestHost::M, 'show-source -d'))
    ...
    context "when given a 'broken' module" do
      ...
      it "skips over the module" do
        output = pro_grammar_eval('show-source -d TestHost::M')
    ...
    context "when provided a class without docs that has a superclass with docs" do
      ...
      it "shows the docs of the superclass" do
        expect(pro_grammar_eval(binding, 'show-source -d Child')).to match(/parent class/)
      end

      it "shows a warning about superclass reversion" do
        expect(pro_grammar_eval(binding, 'show-source -d Child')).to match(
    ...
    context "when provided a class without docs that has nth superclass with docs" do
      ...
      it "shows the docs of the superclass" do
        expect(pro_grammar_eval(binding, 'show-source -d Child'))
    ...
    context "when the module with docs was included in another module" do
      ...
      it "shows the included module's doc" do
        expect(pro_grammar_eval(binding, 'show-source -d Beta'))
          .to match(/mod module doc/)
      end

      it "shows a warning about the included module reversion" do
        expect(pro_grammar_eval(binding, 'show-source -d Beta')).to match(
    ...
    context "when included module has docs and there are intermediary docless modules" do
      ...
      it "shows nth level included module doc" do
        expect(pro_grammar_eval(binding, 'show-source -d Gamma')).to match(/alpha doc/)
      end

      it "shows a warning about module reversion" do
        expect(pro_grammar_eval(binding, 'show-source -d Gamma')).to match(
    ...
    context "when the --super switch is provided" do
      ...
      context "and when it's passed once" do
        it "finds the super method docs" do
          expect(pro_grammar_eval(binding, 'show-source -d --super @obj.initialize'))
            .to match(/child init/)
        end
      end
      ...
      context "and when it's passed twice" do
        it "finds the parent method docs" do
          expect(pro_grammar_eval(binding, 'show-source -d -ss @obj.initialize'))
      ...
      context "and when it's passed thrice" do
        it "finds the grandparent method docs" do
          expect(pro_grammar_eval(binding, 'show-source -d -sss @obj.initialize'))
            .to match(/parent init/)
        end
      end
      ...
      context "and when the super method doesn't exist" do
        it "raises ProGrammar::CommandError" do
          expect { pro_grammar_eval(binding, 'show-source -d -ssss @obj.initialize') }
            .to raise_error(ProGrammar::CommandError)
        end
      end
      ...
      context "and when the explicit argument is not provided" do
        ...
        it "finds super method docs without explicit method argument" do
          # son init
          def son.initialize
            pro_grammar_eval(binding, 'show-source -d --super')
          end
        ...
        it "finds super method docs with multiple `--super` switches" do
          ...
          # son init
          def son.initialize
            pro_grammar_eval(binding, 'show-source -d --super --super')
          end
    ...
    describe "code highlighting" do
      context "when there's code in the docs" do
        ...
        it "highlights the code" do
          expect(pro_grammar_eval(binding, 'show-source -d klass#initialize'))
          ...
          expect(
            pro_grammar_eval(
              binding,
              'pro_grammar_instance.color = true',
    ...
    describe "the --all switch behavior" do
      ...
      context "when there are monkeypatches in different files" do
        ...
        it "shows them" do
          result = pro_grammar_eval(binding, 'show-source -d TestClass -a')
      ...
      context "when --all is not used but there are multiple monkeypatches" do
        ...
        it "correctly displays the number of monkeypatches" do
          result = pro_grammar_eval(binding, 'show-source -d TestClass')
    ...
    context "when used against a command" do
      ...
      it "displays help for a specific command" do
        expect(pro_grammar_eval(binding, 'show-source -d ls')).to match(/Usage: ls/)
      end

      it "displays help for a regex command with a \"listing\"" do
        command_set.command(/bar(.*)/, 'Test listing', listing: 'foo') {}
        expect(pro_grammar_eval(binding, 'show-source -d foo')).to match(/Test listing/)
      end
      ...
      it "displays help for a command with a spaces in its name" do
        ...
        expect(pro_grammar_eval(binding, 'show-source -d command with spaces')).to match(
      ...
      describe "class commands" do
        before do
          # pretty pink pincers
          class LobsterLady < ProGrammar::ClassCommand
            match 'lobster-lady'
            description 'nada.'
            def process
              'lobster'
        ...
        context "when looking up by command name" do
          it "displays help" do
            expect(pro_grammar_eval('show-source -d lobster-lady')).to match(/nada/)
          end
        end
        ...
        context "when class is used (rather than command name) is used for lookup" do
          it "displays actual preceding comment for a class command" do
            expect(pro_grammar_eval('show-source -d LobsterLady')).to match(/pretty pink pincers/)
          end
        end
      end
    end
