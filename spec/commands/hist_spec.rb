...
describe "hist" do
  before do
    ...
    @hist = ProGrammar.history

    @str_output = StringIO.new
    @t = pro_grammar_tester history: @hist do
  ...
  it 'should replay a range of history correctly (range of commands)' do
    ...
    @t.eval("hist --replay 0..2")
    stack = @t.eval("Pad.stack = pro_grammar_instance.binding_stack.dup")
  ...
  it "should disallow execution of `--replay <i>` when CommandError raised" do
    ...
    expect { @t.eval "hist --replay 3" }.to raise_error ProGrammar::CommandError
    expect(@t.eval("a")).to eq 2
    expect(@t.eval("hist").lines.to_a.size).to eq 5
  end

  it "excludes ProGrammar commands from the history with `-e` switch" do
    @hist.push('a = 20')
    @hist.push('ls')
    expect(pro_grammar_eval('hist -e')).to eq "1: a = 20\n"
  end

  describe "sessions" do
    before do
      @old_file = ProGrammar.config.history_file
      ProGrammar.config.history_file = File.expand_path('spec/fixtures/pro_grammar_history')
      @hist.load
    end
