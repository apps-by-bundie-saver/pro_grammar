...
describe "cat" do
  before do
    ...
    @t = pro_grammar_tester do
      def insert_nil_input
        @pro_grammar.update_input_history(nil)
      end
  ...
  describe "when invoked without arguments" do
    it 'should display an error message' do
      expect { @t.eval 'cat' }
        .to raise_error(ProGrammar::CommandError, /Must provide a filename/)
    end
  end
    ...
    it 'cat --ex should display repl code that generated exception' do
      @t.eval(unindent(<<-LAST_EXCEPTION))
        begin
          ...
          pro_grammar_instance.last_exception = e
        end
      LAST_EXCEPTION
  ...
  describe "with --ex N" do
    ...
    it 'should show error when backtrace level out of bounds' do
      ...
      expect { @t.eval('cat --ex 3') }
        .to raise_error(ProGrammar::CommandError, /out of bounds/)
    end
    ...
    it(
      ...
      temp_files = []
      3.times do |i|
        temp_files << Tempfile.new(['pro_grammar', '.rb'])
