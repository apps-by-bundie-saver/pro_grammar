...
describe "whereami" do
  it 'should work with methods that have been undefined' do
    ...
    expect(pro_grammar_eval(Pad.binding, 'whereami')).to match(/self[.]blimey!/)
  ...
  it 'should work in objects with no method methods' do
    class Cor
      def blimey!
        pro_grammar_eval(binding, 'whereami')
      end
  ...
  it 'should properly set _file_, _line_ and _dir_' do
    class Cor
      def blimey!
        pro_grammar_eval(binding, 'whereami', '_file_')
      end
    end
  ...
  it 'should work in BasicObjects' do
    ...
    expect(pro_grammar_eval(cor, 'whereami')).to match(/::Kernel.binding [#] omnom/)
  end
  ...
  it(
    ...
    expect(Cor.instance_method(:blimey!).source).to match(/pro_grammar_eval/)
  ...
  #         Pad.tester = pro_grammar_tester(binding)
  ...
  it 'should show code window (not just method source) if parameter passed to whereami' do
    class Cor
      def blimey!
        pro_grammar_eval(binding, 'whereami 3')
      end
    end
  ...
  it 'should show entire method when -m option used' do
    old_size = ProGrammar.config.default_window_size
    ProGrammar.config.default_window_size = 1
    old_cutoff = ProGrammar::Command::Whereami.method_size_cutoff
    ProGrammar::Command::Whereami.method_size_cutoff = 1
    class Cor
      def blimey!
        @foo = 1
        @bar = 2
        pro_grammar_eval(binding, 'whereami -m')
      end
    end
    ProGrammar::Command::Whereami.method_size_cutoff = old_cutoff
    ProGrammar.config.default_window_size = old_size
  ...
  it 'should show entire file when -f option used' do
    class Cor
      def blimey!
        pro_grammar_eval(binding, 'whereami -f')
      end
    end
    result = Cor.new.blimey!
    Object.remove_const(:Cor)
  ...
  describe "-c" do
    it 'should show class when -c option used, and locate correct candidate' do
      ...
      class Cor
        def blimey!
          pro_grammar_eval(binding, 'whereami -c')
        end
      end
      out = Cor.new.blimey!
      Object.remove_const(:Cor)
    ...
    it 'should show class when -c option used, and binding is outside a method' do
      class Cor
        ...
        def blimey; end
        out = pro_grammar_eval(binding, 'whereami -c')
        expect(out).to match(/class Cor/)
        expect(out).to match(/blimey/)
      end
      Object.remove_const(:Cor)
  ...
  it 'should not show line numbers or marker when -n switch is used' do
    class Cor
      def blimey!
        pro_grammar_eval(binding, 'whereami -n')
      end
    end
    out = Cor.new.blimey!
  ...
  it(
    'uses ProGrammar.config.default_window_size for window size when outside a method context'
  ) do
    old_size = ProGrammar.config.default_window_size
    ProGrammar.config.default_window_size = 1
    _foo = :litella
    _foo = :pig
    out = pro_grammar_eval(binding, 'whereami')
    _foo = :punk
    _foo = :sanders
  ...
  it "should work at the top level" do
    expect(pro_grammar_eval(ProGrammar.toplevel_binding, 'whereami')).to match(
      /At the top level/
    )
  end

  it "should work inside a class" do
    expect(pro_grammar_eval(ProGrammar, 'whereami')).to match(/Inside ProGrammar/)
  end

  it "should work inside an object" do
    expect(pro_grammar_eval(Object.new, 'whereami')).to match(/Inside #<Object/)
  end
end
