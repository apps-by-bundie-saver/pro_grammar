...
describe "edit" do
  before do
    @old_editor = ProGrammar.config.editor
    @file = @line = @contents = nil
    ProGrammar.config.editor = lambda do |file, line|
      @file = file
      @line = line
  ...
  after do
    ProGrammar.config.editor = @old_editor
  end
  ...
  describe "with FILE" do
    ...
    it "should not allow patching any known kind of file" do
      ["file.rb", "file.c", "file.py", "file.yml", "file.gemspec",
       "/tmp/file", "\\\\Temp\\\\file"].each do |file|
        expect { pro_grammar_eval "edit -p #{file}" }
          .to raise_error(NotImplementedError, /Cannot yet patch false objects!/)
      end
    end

    it "should invoke ProGrammar.config.editor with absolutified filenames" do
      pro_grammar_eval 'edit lib/pro_grammar.rb'
      expect(@file).to eq File.expand_path('lib/pro_grammar.rb')

      pro_grammar_eval "edit #{@tf_path}"
      expect(@file).to eq @tf_path
    end

    it "should guess the line number from a colon" do
      pro_grammar_eval 'edit lib/pro_grammar.rb:10'
      expect(@line).to eq 10
    end

    it "should use the line number from -l" do
      pro_grammar_eval 'edit -l 10 lib/pro_grammar.rb'
      expect(@line).to eq 10
    end

    it "should not delete the file!" do
      pro_grammar_eval 'edit Rakefile'
    ...
    if respond_to?(:require_relative, true)
      it "should work with require relative" do
        ProGrammar.config.editor = lambda { |file, _line|
    ...
    describe do
      ...
      it "should reload a non-ruby file if -r is given" do
        temp_file('.pro_grammarrc') do |tf|
          counter = Pad.counter
          path    = tf.path

          pro_grammar_eval "edit -r #{path}"
    ...
    describe do
      before do
        @reloading = nil
        ProGrammar.config.editor = lambda do |file, line, reloading|
          @file = file
          @line = line
          @reloading = reloading
          nil
      ...
      it "should pass the editor a reloading arg" do
        ...
        pro_grammar_eval 'edit -n lib/pro_grammar.rb'
  ...
  describe "with --ex" do
    before do
      @t = pro_grammar_tester do
        def last_exception=(exception)
          @pro_grammar.last_exception = exception
        end

        def last_exception
          @pro_grammar.last_exception
        end
      end
    end
    ...
    describe "with a real file" do
      before do
        @tf = Tempfile.new(["pro_grammar", ".rb"])
        @path = @tf.path
        @tf << "_foo = 1\n_bar = 2\nraise RuntimeError"
        @tf.flush
      ...
      it "should reload the file" do
        @t.pro_grammar.config.editor = lambda { |file, _line|
      ...
      it 'edits the exception even when in a patched method context' do
        source_location = nil
        ProGrammar.config.editor = lambda { |file, line|
          source_location = [file, line]
          nil
        }

        Pad.le = @t.last_exception
        redirect_pro_grammar_io(InputTester.new("def broken_method", "binding.pro_grammar", "end",
                                        "broken_method",
                                        "pro_grammar_instance.last_exception = Pad.le",
    ...
    describe "with --ex NUM" do
      before do
        @t.pro_grammar.config.editor = proc do |file, line|
          @__ex_file__ = file
          @__ex_line__ = line
          nil
        end
      ...
      it 'should display error message when backtrace level is invalid' do
        expect { @t.eval 'edit -n --ex 4' }.to raise_error ProGrammar::CommandError
      end
    end
  end
  ...
  describe "without FILE" do
    ...
    it "should write the evaluated command to history" do
      ...
      @t.process_command 'edit'
      expect(ProGrammar.history.to_a.last).to eq quote
    end
  end

  describe "with --in" do
    it "should edit the nth line of _in_" do
      pro_grammar_eval '10', '11', 'edit --in -2'
      expect(@contents).to eq "10\n"
    end

    it "should edit the last line if no argument is given" do
      pro_grammar_eval '10', '11', 'edit --in'
      expect(@contents).to eq "11\n"
    end

    it "should edit a range of lines if a range is given" do
      pro_grammar_eval "10", "11", "edit -i 1,2"
      expect(@contents).to eq "10\n11\n"
    end

    it "should edit a multi-line expression as it occupies one line of _in_" do
      pro_grammar_eval "class #{1.class}\n  def invert; -self; end\nend", "edit -i 1"
      expect(@contents).to eq "class #{1.class}\n  def invert; -self; end\nend\n"
    end

    it "should not work with a filename" do
      expect { pro_grammar_eval 'edit ruby.rb -i' }.to raise_error(
        ProGrammar::CommandError, /Only one of --ex, --temp, --in, --method and FILE/
      )
    end

    it "should not work with nonsense" do
      expect { pro_grammar_eval 'edit --in three' }.to raise_error(
        ProGrammar::CommandError, /Not a valid range: three/
      )
    end
  end
  ...
  describe 'when editing a method by name' do
    def use_editor(tester, options)
      tester.pro_grammar.config.editor = lambda do |filename, _line|
    ...
    it 'uses patch editing on methods that were previously patched' do
      # initial definition
      tester   = pro_grammar_tester binding
      filename = __FILE__
      line     = __LINE__ + 2
      klass    = Class.new do
        def m; 1; end
  ...
  describe "old edit-method tests now migrated to edit" do
    describe "on a method defined in a file" do
      before do
        ...
        @tempfile = Tempfile.new(['pro_grammar', '.rb'])
      ...
      describe 'with -p' do
        before do
          ProGrammar.config.editor = lambda do |file, _line|
        ...
        it "should successfully replace a method on an instance" do
          pro_grammar_eval 'instance = X.new', 'edit -p instance.x'
        ...
        it "should preserve module nesting" do
          pro_grammar_eval 'edit -p X::B#foo'
      ...
      describe 'on an aliased method' do
        ...
        it "should change the alias, but not the original, without breaking super" do
          pro_grammar_eval 'edit -p X#c'

          expect(ProGrammar::Method.from_str("X#c").alias?).to eq true
  ...
  describe "--method flag" do
    ...
    it 'should edit method context' do
      ...
      t = pro_grammar_tester(BinkyWink.new.m2)
    ...
    it 'errors when cannot find method context' do
      ...
      t = pro_grammar_tester(BinkyWink.new.m1)
      expect { t.process_command "edit -m -n" }
        .to raise_error(ProGrammar::CommandError, /Cannot find a file for/)
    end

    it 'errors when a filename arg is passed with --method' do
      expect { @t.process_command "edit -m ProGrammar#repl" }
        .to raise_error(ProGrammar::CommandError, /Only one of/)
    end
  end
