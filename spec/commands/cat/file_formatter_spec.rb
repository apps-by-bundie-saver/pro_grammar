...
describe ProGrammar::Command::Cat::FileFormatter do
  before do
    @p   = ProGrammar.new
    @opt = ProGrammar::Slop.new
  end

  describe "#file_and_line" do
    before do
      expect(ProGrammar::Code).to receive(:from_file)
    end

    describe "windows filesystem" do
      it "parses '/'style absolute path without line_num" do
        file_with_embedded_line = "C:/Ruby193/pro_grammar_instance.rb"
        ff = described_class.new(file_with_embedded_line, @p, @opt)
        file_name, line_num = ff.file_and_line
        expect(file_name).to eq "C:/Ruby193/pro_grammar_instance.rb"
        expect(line_num).to eq nil
      end

      it "parses '/'style absolute path with line_num" do
        file_with_embedded_line = "C:/Ruby193/pro_grammar_instance.rb:2"
        ff = described_class.new(file_with_embedded_line, @p, @opt)
        file_name, line_num = ff.file_and_line
        expect(file_name).to eq "C:/Ruby193/pro_grammar_instance.rb"
    ...
    describe "UNIX-like filesystem" do
      it "parses absolute path without line_num" do
        file_with_embedded_line = "/Ruby193/pro_grammar_instance.rb"
        ff = described_class.new(file_with_embedded_line, @p, @opt)
        file_name, line_num = ff.file_and_line
        expect(file_name).to eq "/Ruby193/pro_grammar_instance.rb"
        expect(line_num).to eq nil
      end

      it "parses absolute path with line_num" do
        file_with_embedded_line = "/Ruby193/pro_grammar_instance.rb:2"
        ff = described_class.new(file_with_embedded_line, @p, @opt)
        file_name, line_num = ff.file_and_line
        expect(file_name).to eq "/Ruby193/pro_grammar_instance.rb"
        expect(line_num).to eq 2
    ...
    it "parses relative path without line_num" do
      file_with_embedded_line = "pro_grammar_instance.rb"
      ff = described_class.new(file_with_embedded_line, @p, @opt)
      file_name, line_num = ff.file_and_line
      expect(file_name).to eq "pro_grammar_instance.rb"
      expect(line_num).to eq nil
    end

    it "parses relative path with line_num" do
      file_with_embedded_line = "pro_grammar_instance.rb:2"
      ff = described_class.new(file_with_embedded_line, @p, @opt)
      file_name, line_num = ff.file_and_line
      expect(file_name).to eq "pro_grammar_instance.rb"
      expect(line_num).to eq 2
