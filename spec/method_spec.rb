...
describe ProGrammar::Method do
  it "should use String names for compatibility" do
    klass = Class.new { def hello; end }
    expect(ProGrammar::Method.new(klass.instance_method(:hello)).name).to eq "hello"
  end

  describe ".from_str" do
    it 'looks up instance methods if no methods available and no options provided' do
      klass = Class.new { def hello; end }
      meth = ProGrammar::Method.from_str(:hello, ProGrammar.binding_for(klass))
    ...
    it 'should look up instance methods if "instance-methods"  option provided' do
      ...
      meth = ProGrammar::Method.from_str(
        :hello, ProGrammar.binding_for(klass), "instance-methods" => true
      )
      expect(meth).to eq klass.instance_method(:hello)
    end
    ...
    it 'should look up methods if :methods  option provided' do
      ...
      meth = ProGrammar::Method.from_str(:hello, ProGrammar.binding_for(klass), methods: true)
      expect(meth).to eq klass.method(:hello)
    end
    ...
    it 'should look up instance methods using the Class#method syntax' do
      ...
      meth = ProGrammar::Method.from_str("klass#hello", ProGrammar.binding_for(binding))
      expect(meth).to eq klass.instance_method(:hello)
    end
    ...
    it(
      ...
      meth = ProGrammar::Method.from_str("_klass#hello", ProGrammar.binding_for(binding))
    ...
    it 'should look up methods using klass.new.method syntax' do
      ...
      meth = ProGrammar::Method.from_str("_klass.new.hello", ProGrammar.binding_for(binding))
      expect(meth.name).to eq "hello"
    end

    it 'should take care of corner cases like mongo[] e.g Foo::Bar.new[]- issue 998' do
      _klass = Class.new { def []; :hello; end }
      meth = ProGrammar::Method.from_str("_klass.new[]", ProGrammar.binding_for(binding))
      expect(meth.name).to eq "[]"
    end

    it 'should take care of cases like $ mongo[] - issue 998' do
      f = Class.new { def []; :hello; end }.new
      meth = ProGrammar::Method.from_str("f[]", ProGrammar.binding_for(binding))
    ...
    it 'should look up instance methods using klass.meth#method syntax' do
      ...
      meth = ProGrammar::Method.from_str("_klass.meth#initialize", ProGrammar.binding_for(binding))
      expect(meth.name).to eq "initialize"
    end

    it 'should look up methods using instance::bar syntax' do
      _klass = Class.new { def self.meth; Class.new; end }
      meth = ProGrammar::Method.from_str("_klass::meth", ProGrammar.binding_for(binding))
      expect(meth.name).to eq "meth"
    end

    it 'should not raise an exception if receiver does not exist' do
      expect { ProGrammar::Method.from_str("random_klass.meth", ProGrammar.binding_for(binding)) }
        .to_not raise_error
    end
  end
  ...
  describe '.from_binding' do
    it 'should be able to pick a method out of a binding' do
      ...
      expect(ProGrammar::Method.from_binding(klass).name).to eq('foo')
    end

    it 'should NOT find a method from the toplevel binding' do
      expect(ProGrammar::Method.from_binding(TOPLEVEL_BINDING)).to eq nil
    end
    ...
    it "should find methods that have been undef'd" do
      ...
      m = ProGrammar::Method.from_binding(c.bar)
    ...
    it 'should find the super method correctly' do
      ...
      g = b.new.gag33
      m = ProGrammar::Method.from_binding(g)
    ...
    it 'should find the right method if a super method exists' do
      ...
      m = ProGrammar::Method.from_binding(b.new.gag)
    ...
    it "should find the right method from a BasicObject" do
      ...
      m = ProGrammar::Method.from_binding(a.new.gag)
      expect(m.owner).to eq a
    ...
    it 'should find the right method even if it was renamed and replaced' do
      ...
      m = ProGrammar::Method.from_binding(o.borscht)
      expect(m.source).to eq ProGrammar::Method(o.method(:paella)).source
    end
  end
  ...
  describe 'super' do
    it 'should be able to find the super method on a bound method' do
      ...
      zuper = ProGrammar::Method(obj.method(:rar)).super
    ...
    it 'should be able to find the super method of an unbound method' do
      ...
      zuper = ProGrammar::Method(b.instance_method(:rar)).super
      expect(zuper.owner).to eq a
    end
    ...
    it 'should return nil if no super method exists' do
      ...
      expect(ProGrammar::Method(a.instance_method(:rar)).super).to eq nil
    end
    ...
    it 'should be able to find super methods defined on modules' do
      ...
      zuper = ProGrammar::Method(a.new.method(:rar)).super
    ...
    it(
      ...
      zuper = ProGrammar::Method(b.new.method(:rar)).super
      expect(zuper.owner).to eq a
    end
  ...
  describe 'all_from_class' do
    def should_find_method(name)
      expect(ProGrammar::Method.all_from_class(@class).map(&:name)).to include name
    end
    ...
    it 'should attribute overridden methods to the sub-class' do
      ...
      expect(ProGrammar::Method.all_from_class(@class).detect { |x| x.name == 'meth' }.owner)
        .to eq @class
    end
  ...
  describe 'all_from_obj' do
    describe 'on normal objects' do
      def should_find_method(name)
        expect(ProGrammar::Method.all_from_obj(@obj).map(&:name)).to include name
      end
      ...
      it "should not find methods defined on the classes singleton class" do
        ...
        expect(ProGrammar::Method.all_from_obj(@obj).map(&:name)).not_to include 'meth'
      end
    ...
    describe 'on classes' do
      def should_find_method(name)
        expect(ProGrammar::Method.all_from_obj(@class).map(&:name)).to include name
      end
      ...
      it "should not find methods defined within the class" do
        @class = Class.new { def meth; 1; end }
        expect(ProGrammar::Method.all_from_obj(@class).map(&:name)).not_to include 'meth'
      end
      ...
      it "should attribute overridden methods to the sub-class' singleton class" do
        ...
        expect(ProGrammar::Method.all_from_obj(@class).detect { |x| x.name == 'meth' }.owner)
      ...
      it(
        ...
        expect(ProGrammar::Method.all_from_obj(@class).detect { |x| x.name == 'allocate' }.owner)
          .to eq(class << @class; self; end)
      end
    end
    ...
    describe 'method resolution order' do
      ...
      it "should look at a class and then its superclass" do
        expect(ProGrammar::Method.instance_resolution_order(LS::Next))
          .to eq [LS::Next] + ProGrammar::Method.instance_resolution_order(LS::Top)
      end

      it "should include the included modules between a class and its superclass" do
        expect(ProGrammar::Method.instance_resolution_order(LS::Low)).to eq(
          [LS::Low, LS::P, LS::N, LS::M] + ProGrammar::Method.instance_resolution_order(LS::Next)
        )
      end

      it "should not include modules extended into the class" do
        expect(ProGrammar::Method.instance_resolution_order(LS::Bottom)).to eq(
          [LS::Bottom] + ProGrammar::Method.instance_resolution_order(LS::Lower)
        )
      end

      it "should include included modules for Modules" do
        expect(ProGrammar::Method.instance_resolution_order(LS::O)).to eq [LS::O, LS::M]
      end

      it "should include the singleton class of objects" do
        obj = LS::Low.new
        expect(ProGrammar::Method.resolution_order(obj)).to eq(
          [eigen_class(obj)] + ProGrammar::Method.instance_resolution_order(LS::Low)
        )
      end

      it "should not include singleton classes of numbers" do
        target_class = 4.class
        expect(ProGrammar::Method.resolution_order(4)).to eq(
          ProGrammar::Method.instance_resolution_order(target_class)
        )
      end

      it "should include singleton classes for classes" do
        expect(ProGrammar::Method.resolution_order(LS::Low)).to eq(
          [eigen_class(LS::Low)] + ProGrammar::Method.resolution_order(LS::Next)
        )
      end

      it "should include modules included into singleton classes" do
        expect(ProGrammar::Method.resolution_order(LS::Lower)).to eq(
          [eigen_class(LS::Lower), LS::N, LS::M] + ProGrammar::Method.resolution_order(LS::Low)
        )
      end

      it "should include modules at most once" do
        expect(ProGrammar::Method.resolution_order(LS::Bottom).count(LS::M)).to eq 1
      end

      it "should include modules at the point which they would be reached" do
        expect(ProGrammar::Method.resolution_order(LS::Bottom)).to eq(
          [eigen_class(LS::Bottom), LS::O] + ProGrammar::Method.resolution_order(LS::Lower)
        )
      end

      it(
        "includes the ProGrammar::Method.instance_resolution_order of Class after " \
        "the singleton classes"
      ) do
        singleton_classes = [
          eigen_class(LS::Top), eigen_class(Object), eigen_class(BasicObject),
          *ProGrammar::Method.instance_resolution_order(Class)
        ]
        expect(ProGrammar::Method.resolution_order(LS::Top)).to eq(singleton_classes)
      end
    end
  end
  ...
  describe 'method_name_from_first_line' do
    it 'should work in all simple cases' do
      meth = ProGrammar::Method.new(nil)
  ...
  describe 'method aliases' do
    ...
    it 'should be able to find method aliases' do
      meth = ProGrammar::Method(@class.new.method(:eat))
    ...
    it 'should return an empty Array if cannot find aliases' do
      meth = ProGrammar::Method(@class.new.method(:eruct))
    ...
    it 'should find aliases for top-level methods' do
      ...
      meth = ProGrammar::Method.new(method(:my_top_level_method))
    ...
    it 'should be able to find aliases for methods implemented in C' do
      meth = ProGrammar::Method({}.method(:key?))
      aliases = Set.new(meth.aliases)
  ...
  describe '.signature' do
    ...
    if Gem::Version.new(RUBY_VERSION) >= Gem::Version.new("2.1")
      it 'should print the name of keyword args, with :? after the arg name' do
        ...
        RUBY
        signature = ProGrammar::Method.new(@class.method(:keyword)).signature
      ...
      it 'should print the name of keyword args, with : after the arg name' do
        ...
        RUBY
        signature = ProGrammar::Method.new(@class.method(:required_keyword)).signature
