...
class ProGrammar
  ...
  class History
    def self.default_file
      history_file =
        if (xdg_home = ProGrammar::Env['XDG_DATA_HOME'])
          # See XDG Base Directory Specification at
          # https://standards.freedesktop.org/basedir-spec/basedir-spec-0.8.html
          xdg_home + '/pro_grammar/pro_grammar_history'
        elsif File.exist?(File.expand_path('~/.pro_grammar_history'))
          '~/.pro_grammar_history'
        else
          '~/.local/share/pro_grammar/pro_grammar_history'
        end
    ...
    # @return [Fixnum] Number of lines in history when ProGrammar first loaded.
    ...
    def push(line)
      ...
      @history << line
      @history_line_count += 1
      @saver.call(line) if !should_ignore?(line) && ProGrammar.config.history_save
    ...
    # The default saver. Appends the given line to `ProGrammar.config.history_file`.
    ...
    def history_file_path
      File.expand_path(@file_path || ProGrammar.config.history_file)
    end
