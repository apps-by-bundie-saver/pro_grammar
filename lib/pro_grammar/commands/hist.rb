...
class ProGrammar
  class Command
    class Hist < ProGrammar::ClassCommand
      match 'hist'
      group 'Editing'
      ...
      def options(opt)
        ...
        opt.on :e, :'exclude-pro_grammar', "Exclude ProGrammar commands from the history"
      ...
      def process
        ...
        if opts.present?(:'exclude-pro_grammar')
      ...
      def process_replay
        ...
        replay_sequence.lines.each do |line|
          pro_grammar_instance.eval line, generated: true
        end
      end

      # Checks +replay_sequence+ for the presence of neighboring replay calls.
      # @example
      #   [1] pro_grammar(main)> hist --show 46894
      #   46894: hist --replay 46675..46677
      #   [2] pro_grammar(main)> hist --show 46675..46677
      #   46675: 1+1
      #   46676: a = 100
      #   46677: hist --tail
      #   [3] pro_grammar(main)> hist --replay 46894
      #   Error: Replay index 46894 points out to another replay call:
      #   `hist -r 46675..46677`
      #   [4] pro_grammar(main)>
      #
      # @raise [ProGrammar::CommandError] If +replay_sequence+ contains another
      #   "hist --replay" call
      ...
      #
      # @return [ProGrammar::Code] if it finds `--all` (or `-a`) switch, returns all
      #   entries in history. Without the switch returns only the entries from the
      #   current ProGrammar session.
      def find_history
        h = if opts.present?(:all)
              ProGrammar.history.to_a
            else
              ProGrammar.history.to_a.last(ProGrammar.history.session_line_count)
            end

        ProGrammar::Code(ProGrammar.history.filter(h[0..-2]))
      end
    end

    ProGrammar::Commands.add_command(ProGrammar::Command::Hist)
    ProGrammar::Commands.alias_command 'history', 'hist'
  end
end
