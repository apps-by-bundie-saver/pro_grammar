...
class ProGrammar
  class Command
    class Wtf < ProGrammar::ClassCommand
      match(/wtf([?!]*)/)
      group 'Context'
      ...
      def process
        unless pro_grammar_instance.last_exception
          raise ProGrammar::CommandError, "No most-recent exception"
        end
      ...
      def unwind_exceptions
        exception_list = []
        exception = pro_grammar_instance.last_exception
      ...
      def format_backtrace(backtrace)
        ...
        ProGrammar::Code.new(lines.compact, 0, :text).with_line_numbers.to_s
      end
    ...
    ProGrammar::Commands.add_command(ProGrammar::Command::Wtf)
  end
end
