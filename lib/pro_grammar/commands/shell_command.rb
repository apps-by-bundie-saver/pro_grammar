...
class ProGrammar
  class Command
    class ShellCommand < ProGrammar::ClassCommand
      match(/\.(.*)/)
      ...
      def process(cmd)
        if cmd =~ /^cd\s*(.*)/i
          ...
          if command_block
            ...
            pro_grammar_instance.config.system.call(output, cmd, pro_grammar_instance)
          end
        end
      end
    ...
    ProGrammar::Commands.add_command(ProGrammar::Command::ShellCommand)
  end
end
