...
class ProGrammar
  class Command
    class CodeCollector
      ...
      def initialize(args, opts, pro_grammar_instance)
        @args = args
        @opts = opts
      ...
      def self.inject_options(opt)
        ...
        opt.on :o, :out, "Select lines from ProGrammar's output result history. " \
                         "Takes an index or range",
               optional_argument: true, as: Range, default: -5..-1 do |r|
          output_result_ranges << (r || (-5..-1))
        end
        opt.on :i, :in, "Select lines from ProGrammar's input expression history. " \
                        "Takes an index or range",
               optional_argument: true, as: Range, default: -5..-1 do |r|
        ...
        @content ||=
          begin
            ...
            content = if opts.present?(:o)
                        pro_grammar_output_content
                      elsif opts.present?(:i)
                        pro_grammar_input_content
      ...
      # The code object
      #
      # @return [ProGrammar::WrappedModule, ProGrammar::Method, ProGrammar::Command]
      def code_object
        ProGrammar::CodeObject.lookup(obj_name, pro_grammar_instance, super: opts[:super])
      end
      ...
      # The selected `pro_grammar_instance.output_ring` as a string, as specified by
      # the `-o` switch.
      #
      # @return [String]
      def pro_grammar_output_content
        pro_grammar_array_content_as_string(
          pro_grammar_instance.output_ring,
          self.class.output_result_ranges,
          &:pretty_inspect
        )
      end

      # The selected `pro_grammar_instance.input_ring` as a string, as specified by
      # the `-i` switch.
      #
      # @return [String]
      def pro_grammar_input_content
        pro_grammar_array_content_as_string(
          pro_grammar_instance.input_ring, self.class.input_expression_ranges
        ) { |v| v }
      end
      ...
      def pro_grammar_array_content_as_string(array, ranges)
        all = ''
