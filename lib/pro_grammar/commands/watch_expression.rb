...
class ProGrammar
  class Command
    class WatchExpression < ProGrammar::ClassCommand
      match 'watch'
      group 'Context'
      ...
      banner <<-'BANNER'
        ...
        It will be re-evaluated every time you hit enter in pro_grammar. If its value has
        ...
        a method inside pro_grammar and want to check that it gives the right answers
      ...
      # TODO: fix arguments.
      # https://github.com/pro_grammar/pro_grammar/commit/b031df2f2f5850ee6e9018f33d35f3485a9b0423
      def add_expression(_arguments)
        expressions << Expression.new(pro_grammar_instance, target, arg_string)
        output.puts "Watching #{Code.new(arg_string).highlighted}"
      end

      def add_hook
        hook = [:after_eval, :watch_expression]
        return if pro_grammar_instance.hooks.hook_exists?(*hook)

        pro_grammar_instance.hooks.add_hook(*hook) do |_, pro_grammar_instance|
          eval_and_print_changed pro_grammar_instance.output
        end
      end
    end

    ProGrammar::Commands.add_command(ProGrammar::Command::WatchExpression)
  end
end
