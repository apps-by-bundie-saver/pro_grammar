...
class ProGrammar
  class Command
    class ShowInfo < ProGrammar::ClassCommand
      extend ProGrammar::Helpers::BaseHelpers
      ...
      def process
        code_object = ProGrammar::CodeObject.lookup(obj_name, pro_grammar_instance, super: opts[:super])
        ...
        if !obj_name && code_object.c_module? && !opts[:all]
          result = "You're inside an object, whose class is defined by means of " \
                   "the C Ruby API.\nProGrammar cannot display the information for this " \
                   "class."
        ...
        pro_grammar_instance.pager.page result
      end
      ...
      def code_object_header(code_object, line_num)
        if code_object.real_method_object?
          ...
          # It sucks we have to test for both ProGrammar::WrappedModule and
          # WrappedModule::Candidate, probably indicates a deep refactor needs
          # to happen in those classes.
        elsif code_object.is_a?(ProGrammar::WrappedModule) ||
              code_object.is_a?(ProGrammar::WrappedModule::Candidate)
      ...
      def show_all_modules?(code_object)
        code_object.is_a?(ProGrammar::WrappedModule) && opts.present?(:all)
      end
        ...
        if input =~ /([^ ]*)#([a-z0-9_]*)\z/
          ...
          methods =
            begin
              ...
              ProGrammar::Method.all_from_class(binding.eval(prefix))
