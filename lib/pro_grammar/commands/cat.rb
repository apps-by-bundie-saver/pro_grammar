...
class ProGrammar
  class Command
    class Cat < ProGrammar::ClassCommand
      match 'cat'
      group 'Input and Output'
      description "Show code from a file, ProGrammar's input buffer, or the last exception."
      ...
      banner <<-'BANNER'
        ...
        last exception, or an expression from ProGrammar's input history.
      ...
      def options(opt)
        ...
        opt.on :i, :in, "Show one or more entries from ProGrammar's expression history",
        ...
        output =
          if opts.present?(:ex)
            ExceptionFormatter.new(
              pro_grammar_instance.last_exception, pro_grammar_instance, opts
            ).format
          elsif opts.present?(:in)
            InputExpressionFormatter.new(pro_grammar_instance.input_ring, opts).format
          else
            FileFormatter.new(args.first, pro_grammar_instance, opts).format
          end

        pro_grammar_instance.pager.page output
      end
    ...
    ProGrammar::Commands.add_command(ProGrammar::Command::Cat)
  end
end
