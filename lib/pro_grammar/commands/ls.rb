  ...
  class Command
    class Ls < ProGrammar::ClassCommand
      DEFAULT_OPTIONS = {
        ...
        pro_grammar_var_color: :default, # e.g. _, pro_grammar_instance, _file_
      ...
      banner <<-'BANNER'
        ...
        ls shows you which methods, constants and variables are accessible to ProGrammar. By
        ...
        The colours used are configurable using ProGrammar.config.ls.*_color, and the separator
        is ProGrammar.config.ls.separator.

        ProGrammar.config.ls.ceiling is used to hide methods defined higher up in the
      ...
      def options(opt)
        ...
        opt.on :v, :verbose, "Show methods and constants on all super-classes " \
                             "(ignores ProGrammar.config.ls.ceiling)"
      ...
      def process
        ...
        pro_grammar_instance.pager.page ls_entity.entities_table
      end
      ...
      def raise_errors_if_arguments_are_weird
        error_list.each do |message, option, invalid_expr|
          raise ProGrammar::CommandError, message if opts.present?(option) && invalid_expr
        end
      end
    end

    ProGrammar::Commands.add_command(ProGrammar::Command::Ls)
  end
end
