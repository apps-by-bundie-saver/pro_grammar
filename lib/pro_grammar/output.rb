...
class ProGrammar
  class Output
    ...
    def initialize(pro_grammar_instance)
      @output = pro_grammar_instance.config.output
      @color = pro_grammar_instance.config.color
    end
    ...
    def decolorize_maybe(str)
      ...
      ProGrammar::Helpers::Text.strip_color(str)
    end
    ...
    def io_console_size
      return if ProGrammar::Helpers::Platform.jruby?
    ...
    def env_size
      size = [ProGrammar::Env['LINES'] || ProGrammar::Env['ROWS'], ProGrammar::Env['COLUMNS']]
    ...
    def ansicon_env_size
      return unless ProGrammar::Env['ANSICON'] =~ /\((.*)x(.*)\)/
