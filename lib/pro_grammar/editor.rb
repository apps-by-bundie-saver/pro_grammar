...
class ProGrammar
  class Editor
    def self.default
      if (visual = ProGrammar::Env['VISUAL'])
        return visual
      end

      if (editor = ProGrammar::Env['EDITOR'])
        return editor
      end
    ...
    def invoke_editor(file, line, blocking = true)
      unless pro_grammar_instance.config.editor
        raise CommandError,
              "Please set ProGrammar.config.editor or export $VISUAL or $EDITOR"
      end
    ...
    def build_editor_invocation_string(file, line, blocking)
      if pro_grammar_instance.config.editor.respond_to?(:call)
        args = [file, line, blocking][0...(pro_grammar_instance.config.editor.arity)]
        pro_grammar_instance.config.editor.call(*args)
      else
        sanitized_file = Helpers::Platform.windows? ? file : Shellwords.escape(file)
        editor = pro_grammar_instance.config.editor
    ...
    def open_editor(editor_invocation)
      # Note we dont want to use ProGrammar.config.system here as that
    ...
    # not to block the process from which they were launched (in this case, ProGrammar).
    ...
    # Get the name of the binary that ProGrammar.config.editor points to.
    #
    ...
    #
    # @example
    #   ProGrammar.config.editor="/home/conrad/bin/textmate -w"
    #   editor_name
    #   # => textmate
    #
    def editor_name
      File.basename(pro_grammar_instance.config.editor).split(" ").first
    end
  end
end
