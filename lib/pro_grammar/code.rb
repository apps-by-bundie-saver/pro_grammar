...
class ProGrammar
  class << self
    # Convert the given object into an instance of `ProGrammar::Code`, if it isn't
    # already one.
    #
    # @param [Code, Method, UnboundMethod, Proc, ProGrammar::Method, String, Array,
    #   IO] obj
    def Code(obj)
      case obj
      when Code
        obj
      when ::Method, UnboundMethod, Proc, ProGrammar::Method
  ...
  # `ProGrammar::Code` is a class that encapsulates lines of source code and their
  ...
  class Code
    class << self
      ...
      # ProGrammar's line buffer.
      #
      # @param [String] filename The name of a file, or "(pro_grammar)".
      ...
      # `::Method`, `UnboundMethod`, `Proc`, or `ProGrammar::Method` object.
      #
      # @param [::Method, UnboundMethod, Proc, ProGrammar::Method] meth The method
      #   object.
      ...
      #   to use (see `ProGrammar::WrappedModule::Candidate` for more information).
      # @param [Integer, nil] start_line The line number to start on, or nil to
      #   use the method's original line numbers.
      # @return [Code]
      def from_module(mod, candidate_rank = 0, start_line = nil)
        candidate = ProGrammar::WrappedModule(mod).candidate(candidate_rank)
    ...
    def nesting_at(line_number)
      ProGrammar::Indent.nesting_at(raw, line_number)
    end
