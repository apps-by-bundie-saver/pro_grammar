...
class ProGrammar
  class Pager
    ...
    # `ProGrammar.config.pager` is enabled). If you want to send text through in
    ...
    # you must rescue `ProGrammar::Pager::StopPaging`. These requirements can be
    # avoided by using `.open` instead.
    def best_available
      if !pro_grammar_instance.config.pager
        NullPager.new(pro_grammar_instance.output)
      elsif !SystemPager.available? || Helpers::Platform.jruby?
        SimplePager.new(pro_grammar_instance.output)
      else
        SystemPager.new(pro_grammar_instance.output)
      end
    end

    # `NullPager` is a "pager" that actually just prints all output as it
    # comes in. Used when `ProGrammar.config.pager` is false.
    class NullPager
      def initialize(out)
        @out = out
      end
