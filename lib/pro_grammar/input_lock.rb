  ...
  class InputLock
    ...
    def self.for(input)
      ...
      global_lock.synchronize do
        input_locks[input] ||= ProGrammar::InputLock.new
      end
    end
    ...
    def with_ownership(&block)
      # If we are in a nested with_ownership() call (nested pro_grammar context), we do nothing.
    ...
    def enter_interruptible_region
      @mutex.synchronize do
        ...
        # thread calls with_ownership() because of a binding.pro_grammar happening in
        # another thread.
