  ...
  module Helpers
    module CommandHelpers
      ...
      def temp_file(ext = '.rb')
        file = Tempfile.new(['pro_grammar', ext])
        yield(file)
      ensure
        file.close(true)
      end
      ...
      def internal_binding?(context)
        ...
        %w[__binding__ __pro_grammar__ class_eval].include?(method_name)
      ...
      def get_method_or_raise(method_name, context, opts = {})
        method = ProGrammar::Method.from_str(method_name, context, opts)
        if !method && method_name
          raise ProGrammar::MethodNotFound, "method '#{method_name}' could not be found."
        end
        ...
        (opts[:super] || 0).times do
          if method.super
            ...
            raise ProGrammar::MethodNotFound,
      ...
      def set_file_and_dir_locals(file_name, pro_grammar = pro_grammar_instance, ctx = target)
        return if !ctx || !file_name

        pro_grammar.last_file = File.expand_path(file_name)
        pro_grammar.inject_local("_file_", pro_grammar.last_file, ctx)

        pro_grammar.last_dir = File.dirname(pro_grammar.last_file)
        pro_grammar.inject_local("_dir_", pro_grammar.last_dir, ctx)
      end
    end
  end
end
