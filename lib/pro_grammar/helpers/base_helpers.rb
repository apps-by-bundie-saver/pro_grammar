...
class ProGrammar
  module Helpers
    module BaseHelpers
      ...
      def find_command(name, set = ProGrammar::Commands)
      ...
      def use_ansi_codes?
        ProGrammar::Helpers::Platform.windows_ansi? ||
          ((term = ProGrammar::Env['TERM']) && term != "dumb")
      end
      ...
      # Send the given text through the best available pager (if ProGrammar.config.pager is
      # enabled). Infers where to send the output if used as a mixin.
      # DEPRECATED.
      def stagger_output(text, _out = nil)
        if defined?(pro_grammar_instance) && pro_grammar_instance
          pro_grammar_instance.pager.page text
        else
          ProGrammar.new.pager.page text
        end
      end
    end
  end
