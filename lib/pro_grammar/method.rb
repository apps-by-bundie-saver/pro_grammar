...
class ProGrammar
  class << self
    # If the given object is a `ProGrammar::Method`, return it unaltered. If it's
    # anything else, return it wrapped in a `ProGrammar::Method` instance.
    def Method(obj)
      if obj.is_a? ProGrammar::Method
        obj
      else
        ProGrammar::Method.new(obj)
      end
    end
  end

  # This class wraps the normal `Method` and `UnboundMethod` classes
  # to provide extra functionality useful to ProGrammar.
  class Method # rubocop:disable Metrics/ClassLength
    ...
    class << self
      ...
      #  `name` doesn't contain any context.
      # @return [ProGrammar::Method, nil] A `ProGrammar::Method` instance containing the
      ...
      rescue ProGrammar::RescuableException
        nil
      end

      # Given a `Binding`, try to extract the `::Method` it originated from and
      # use it to instantiate a `ProGrammar::Method`. Return `nil` if this isn't
      # possible.
      #
      # @param [Binding] binding
      # @return [ProGrammar::Method, nil]
      #
      ...
      def lookup_method_via_binding(
        ...
        ProGrammar.current[:obj] = obj
        ProGrammar.current[:name] = method_name
        receiver = obj.is_a?(Module) ? "Module" : "Kernel"
        target.eval(
          "::#{receiver}.instance_method(:#{method_type})" \
          ".bind(ProGrammar.current[:obj]).call(ProGrammar.current[:name])"
        )
      ensure
        ProGrammar.current[:obj] = ProGrammar.current[:name] = nil
      end

      # Given a `Class` or `Module` and the name of a method, try to
      # instantiate a `ProGrammar::Method` containing the instance method of
      ...
      # a `ProGrammar::Method` containing the method of that name bound to
      ...
      # @return [Array[ProGrammar::Method]]
      ...
      def instance_resolution_order(klass)
        ...
        ([klass] + ProGrammar::Method.safe_send(klass, :ancestors)).uniq
      end
      ...
      def singleton_class_resolution_order(klass)
        ancestors = ProGrammar::Method.safe_send(klass, :ancestors)
    ...
    # Workaround for https://github.com/pro_grammar/pro_grammar/pull/2086
    def_delegators :@method, :owner, :parameters, :receiver

    # A new instance of `ProGrammar::Method` wrapping the given `::Method`,
    ...
    # Get the owner of the method as a ProGrammar::Module
    # @return [ProGrammar::Module]
    def wrapped_owner
      @wrapped_owner ||= ProGrammar::WrappedModule.new(owner)
    end

    # Get underlying object wrapped by this ProGrammar::Method instance
    ...
    def redefine(source)
      ...
      ProGrammar::Method(owner.instance_method(name))
    end
    ...
    # @return [ProGrammar::Method, nil] The wrapped method that is called when you
    #   use "super" in the body of this method.
    def super(times = 1)
      if @method.is_a?(UnboundMethod)
        sup = super_using_ancestors(ProGrammar::Method.instance_resolution_order(owner), times)
      else
        sup = super_using_ancestors(ProGrammar::Method.resolution_order(receiver), times)
        sup &&= sup.bind(receiver)
      end
      ProGrammar::Method.new(sup) if sup
    end
    ...
    # @return [Boolean] Was the method defined within the ProGrammar REPL?
    def pro_grammar_method?
      source_file == ProGrammar.eval_path
    end
    ...
    # @return [Boolean]
    def ==(other)
      return other == @method if other.is_a?(ProGrammar::Method)
    ...
    # @return [Boolean]
    def is_a?(klass)
      (klass == ProGrammar::Method) || @method.is_a?(klass)
    end
    alias kind_of? is_a?
    ...
    def comment
      ProGrammar::Code.from_file(source_file).comment_describing(source_line)
    end

    private

    # @return [YARD::CodeObjects::MethodObject]
    # @raise [CommandError] when the method can't be found or `pro_grammar-doc` isn't installed.
    def pro_grammar_doc_info
      if defined?(ProGrammarDoc)
        ProGrammar::MethodInfo.info_for(@method) ||
          raise(
            CommandError,
        ...
        if Helpers::Platform.mri?
          fail_msg += " Invoke the 'gem-install pro_grammar-doc' ProGrammar command to get " \
    ...
    def ruby_source
      ...
      begin
        code = ProGrammar::Code.from_file(file).expression_at(line)
