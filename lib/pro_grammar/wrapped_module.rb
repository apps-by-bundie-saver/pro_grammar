...
class ProGrammar
  class << self
    # If the given object is a `ProGrammar::WrappedModule`, return it unaltered. If it's
    # anything else, return it wrapped in a `ProGrammar::WrappedModule` instance.
    def WrappedModule(obj)
      if obj.is_a? ProGrammar::WrappedModule
        obj
      else
        ProGrammar::WrappedModule.new(obj)
      end
    end
  end
  ...
  class WrappedModule
    ...
    # @example
    #   ProGrammar::WrappedModule.from_str("ProGrammar::Code")
    def self.from_str(mod_name, target = TOPLEVEL_BINDING)
      ProGrammar::WrappedModule.new(target.eval(mod_name)) if safe_to_evaluate?(mod_name, target)
    ...
    class << self
      ...
      # TODO: see duplication of this method in ProGrammar::CodeObject
    ...
    # @return [Boolean]
    def singleton_class?
      if ProGrammar::Method.safe_send(wrapped, :respond_to?, :singleton_class?)
        ProGrammar::Method.safe_send(wrapped, :singleton_class?)
      else
        wrapped != ProGrammar::Method.safe_send(wrapped, :ancestors).first
      end
    end
    ...
    # interested in.
    # @raise [ProGrammar::CommandError] If documentation cannot be found.
    ...
    # `WrappedModule#candidate` to select the candidate you're
    # interested in.
    # @raise [ProGrammar::CommandError] If source cannot be found.
    ...
    # @raise [ProGrammar::CommandError] If the `rank` is out of range. That
    #   is greater than `number_of_candidates - 1`.
    # @param [Fixnum] rank
    # @return [ProGrammar::WrappedModule::Candidate]
    def candidate(rank)
    ...
    # @return [ProGrammar::WrappedModule, nil] The wrapped module that is the
    #   superclass.
    ...
    def super(times = 1)
      ...
      ProGrammar::WrappedModule(sup) if sup
    end

    private

    # @return [ProGrammar::WrappedModule::Candidate] The candidate with the
    ...
    # @return [Array<Array<ProGrammar::Method>>] The array of `ProGrammar::Method` objects,
    ...
    def all_methods_for(mod)
      ProGrammar::Method.all_from_obj(mod, false) + ProGrammar::Method.all_from_class(mod, false)
    end
    ...
    def lines_for_file(file)
      ...
      @lines_for_file[file] ||=
        if file == ProGrammar.eval_path
          ProGrammar.line_buffer.drop(1)
        else
          File.readlines(file)
        end
    end
