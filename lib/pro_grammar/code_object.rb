...
class ProGrammar
  ...
  # `ProGrammar::Command` will be returned. Alternatively, if the user passes in "ProGrammar#repl" then
  # a `ProGrammar::Method` object will be returned.
  #
  ...
  class CodeObject
    module Helpers
      # we need this helper as some ProGrammar::Method objects can wrap Procs
      # @return [Boolean]
      ...
      def command?
        is_a?(Module) && self <= ProGrammar::Command
      end
    ...
    class << self
      def lookup(str, pro_grammar_instance, options = {})
        co = new(str, pro_grammar_instance, options)
    ...
    def initialize(str, pro_grammar_instance, options = {})
      options = {
        super: 0
      }.merge!(options)
    ...
    def command_lookup
      pro_grammar_instance.commands.find_command_by_match_or_listing(str)
    ...
    def empty_lookup
      ...
      obj = if internal_binding?(target)
              ...
              ProGrammar::WrappedModule(mod)
            else
              ProGrammar::Method.from_binding(target)
            end
    ...
    def default_lookup
      ...
      if safe_to_evaluate?(str) && !looks_like_an_instance_method?(str)
        ...
        if sourcable_object?(obj)
          ProGrammar::Method(obj)
        elsif !obj.is_a?(Module)
          ProGrammar::WrappedModule(obj.class)
        end
      end
    ...
    def method_or_class_lookup
      obj =
        case str
        when /\S+\(\)\z/
          ProGrammar::Method.from_str(str.sub(/\(\)\z/, ''), target) ||
          ProGrammar::WrappedModule.from_str(str, target)
        else
          ProGrammar::WrappedModule.from_str(str, target) ||
          ProGrammar::Method.from_str(str, target)
        end
    ...
    # TODO: see duplication of this method in ProGrammar::WrappedModule
    ...
    def lookup_super(obj, super_level)
      ...
      raise ProGrammar::CommandError, "No superclass found for #{obj.wrapped}" unless sup
