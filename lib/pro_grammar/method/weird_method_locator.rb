...
class ProGrammar
  class Method
    # This class is responsible for locating the *real* `ProGrammar::Method`
    # object captured by a binding.
    #
    # Given a `Binding` from inside a method and a 'seed' ProGrammar::Method object,
    # there are primarily two situations where the seed method doesn't match
    # the Binding:
    # 1. The ProGrammar::Method is from a subclass
    # 2. The ProGrammar::Method represents a method of the same name while the original
    ...
    # ProGrammar::Method and return it, or return nil if we fail.
    class WeirdMethodLocator
      class << self
        ...
        #
        # @param [ProGrammar::Method] method
      ...
      # @param [ProGrammar::Method] method The seed method.
      ...
      # @return [ProGrammar::Method, nil] The ProGrammar::Method that matches the
      #   given binding.
      def find_method
        find_method_in_superclass || find_renamed_method
      end

      # @return [Boolean] Whether the ProGrammar::Method is unrecoverable
      ...
      def target_file
        ...
        pro_grammar_file? ? file : File.expand_path(file)
      end
      ...
      #
      # class A; def b; binding.pro_grammar; end; end
      ...
      #
      # @return [ProGrammar::Method, nil] The ProGrammar::Method representing the
      #   superclass method.
      ...
      def find_renamed_method
        ...
        alias_name && ProGrammar::Method(target_self.method(alias_name))
      end
      ...
      def renamed_method_source_location
        ...
        source_index = lines_for_file(target_file)[0..(target_line - 1)].rindex do |v|
          ProGrammar::Method.method_definition?(method.name, v)
        end
      ...
      def index_to_line_number(index)
        # ProGrammar.line_buffer is 0-indexed
        pro_grammar_file? ? index : index + 1
      end

      def valid_file?(file)
        (File.exist?(file) && !File.directory?(file)) || ProGrammar.eval_path == file
      end

      def lines_for_file(file)
        @lines_for_file ||= {}
        @lines_for_file[file] ||= if ProGrammar.eval_path == file
                                    ProGrammar.line_buffer
                                  else
                                    File.readlines(file)
                                  end
      end
