...
class ProGrammar
  class Method
    class Patcher
      ...
      def cache_key
        "pro_grammar-redefined(0x#{method.owner.object_id.to_s(16)}##{method.name})"
      end
      ...
      def with_method_transaction
        temp_name = "__pro_grammar_#{method.original_name}__"
        method = self.method
      ...
      def wrap_for_owner(source)
        ProGrammar.current[:pro_grammar_owner] = method.owner
        owner_source = definition_for_owner(source)
        visibility_fix = "#{method.visibility} #{method.name.to_sym.inspect}"
        "ProGrammar.current[:pro_grammar_owner].class_eval do; #{owner_source}\n#{visibility_fix}\nend"
      end
      ...
      def wrap_for_nesting(source)
        nesting = ProGrammar::Code.from_file(method.source_file).nesting_at(method.source_line)

        (nesting + [source] + nesting.map { "end" } + [""]).join(";")
      rescue ProGrammar::Indent::UnparseableNestingError
        source
      end
    end
  end
