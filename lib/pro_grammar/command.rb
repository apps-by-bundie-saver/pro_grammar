...
class ProGrammar
  ...
  # {ProGrammar::CommandSet#command} which creates a BlockCommand or
  # {ProGrammar::CommandSet#create_command} which creates a ClassCommand. Please don't
  # use this class directly.
  class Command
    extend Helpers::DocumentationHelpers
    extend CodeObject::Helpers

    include ProGrammar::Helpers::BaseHelpers
    include ProGrammar::Helpers::CommandHelpers
    ...
    class << self
      ...
      def source
        ...
        strip_leading_whitespace(ProGrammar::Code.from_file(file).expression_at(line))
      end
      ...
      def name
        super.to_s == "" ? "#<class(ProGrammar::Command #{match.inspect})>" : super
      end
      ...
      # @param [Hash] options Behavioral options (see {ProGrammar::CommandSet#command})
      # @param [Module] helpers A module of helper functions to be included.
      # @yield optional, used for BlockCommands
      # @return [Class] (a subclass of {ProGrammar::Command})
      ...
      def command_regex
        prefix = convert_to_regex(ProGrammar.config.command_prefix)
      ...
      def group(name = nil)
        @group ||= begin
          name || case ProGrammar::Method(block).source_file
                  when %r{/pro_grammar/.*_commands/(.*).rb}
                    Regexp.last_match(1).capitalize.tr('_', " ")
                  when /(pro_grammar-\w+)-([\d\.]+([\w\.]+)?)/
      ...
      def state
        ProGrammar::CommandState.default.state_for(match)
      end
    end

    # Properties of one execution of a command (passed by {ProGrammar#run_command} as a hash of
    ...
    alias _pro_grammar_= pro_grammar_instance=
    ...
    def initialize(context = {})
      ...
      self.pro_grammar_instance = context[:pro_grammar_instance]
    end
    ...
    def run(command_string, *args)
      command_string = pro_grammar_instance.config.command_prefix.to_s + command_string
    ...
    def _pro_grammar_
      ProGrammar::Warning.warn('_pro_grammar_ is deprecated, use pro_grammar_instance instead')
    ...
    # @return [Hash] ProGrammar commands can store arbitrary state
    ...
    def check_for_command_collision(command_match, arg_string)
      ...
      if collision_type
        output.puts(
          "#{Helpers::Text.bold('WARNING:')} Calling ProGrammar command '#{command_match}', " \
          "which conflicts with a #{collision_type}.\n\n"
        )
      end
    rescue ProGrammar::RescuableException # rubocop:disable Lint/HandleExceptions
    end
    ...
    def process_line(line)
      ...
      if ProGrammar.config.collision_warning
    ...
    def pass_block(arg_string)
      ...
      block_string =
        if !ProGrammar::Code.complete_expression?(prime_string)
          pro_grammar_instance.r(target, prime_string)
        else
          prime_string
        end
    ...
    def find_hooks(event)
      ...
      (hooks || ProGrammar.hooks || self.class.hooks).get_hooks(event_name).values
    end
