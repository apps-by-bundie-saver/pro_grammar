...
# taken from irb
# Implements tab completion for Readline in ProGrammar
class ProGrammar
  class InputCompleter
    ...
    def initialize(input, pro_grammar = nil)
    ...
    def call(str, options = {})
      ...
      if path.call.empty?
        ...
        begin
          target = ProGrammar::ObjectPath.new(path.call, @pro_grammar.binding_stack).resolve.last
        # but if that doesn't work, assume they're doing division with no spaces
        rescue ProGrammar::CommandError
          target = options[:target]
        end
      end
    ...
    def ignored_modules
      ...
      # FIXME: Add ProGrammar here as well?
