...
class ProGrammar
  ...
  class CommandSet
    ...
    # Defines a new ProGrammar command.
    ...
    #   `ProGrammar.config.command_prefix` prefix (if one is defined). Defaults
    #   to true.
    ...
    #   command parameters are separated by whitespace at the ProGrammar prompt.
    # @example
    #   MyCommands = ProGrammar::CommandSet.new do
    ...
    #
    #   # From pro_grammar:
    #   # pro_grammar(main)> pro_grammar_instance.commands = MyCommands
    #   # pro_grammar(main)> greet john
    #   # Good afternoon John!
    #   # pro_grammar(main)> help greet
    #   # Greet somebody
    ...
    #   # From pro_grammar:
    #   # pro_grammar(main)> pro_grammar_instance.commands = MyCommands
    #   # pro_grammar(main)> number-10 john
    #   # hello john, nice number: 10
    #   # pro_grammar(main)> help number
    ...
    def block_command(match, description = "No description.", options = {}, &block)
      ...
      options = ProGrammar::Command.default_options(match).merge!(options)

      @commands[match] = ProGrammar::BlockCommand.subclass(
        match, description, options, helper_module, &block
      )
    end
    alias command block_command

    # Defines a new ProGrammar command class.
    #
    # @param [String, Regexp] match The start of invocations of this command.
    # @param [String] description A description of the command.
    ...
    #
    # @example
    #   ProGrammar::Commands.create_command "echo", "echo's the input", :shellwords => false do
    #     def options(opt)
    ...
    #         raise ProGrammar::CommandError, "-u and -d makes no sense"
    #       end
    #       result = args.join(" ")
    ...
    def create_command(match, description = "No description.", options = {}, &block)
      ...
      @commands[match] = ProGrammar::ClassCommand.subclass(
        match, description, options, helper_module, &block
      )
    ...
    #   will be imported.
    # @return [ProGrammar::CommandSet] Returns the reciever (a command set).
    def import(*sets)
      sets.each do |set|
    ...
    def find_command_by_match_or_listing(match_or_listing)
      cmd = (@commands[match_or_listing] ||
        ProGrammar::Helpers::BaseHelpers.find_command(match_or_listing, @commands))
    ...
    #   ProGrammar.config.commands.alias_command "lM", "ls -M"
    # @example Pass explicit description (overriding default).
    #   ProGrammar.config.commands.alias_command "lM", "ls -M", :desc => "cutiepie"
    ...
    #   ProGrammar.config.commands.rename "dir", "ls", :description => "DOS friendly ls"
    ...
    #   end
    # @example Getting
    #   ProGrammar.config.commands.desc "amend-line"
    ...
    #
    # @param [ProGrammar::Command] command
    #   The command to add.
    #
    # @return [ProGrammar::Command]
    #   Returns the new command (matched with "pattern".)
    #
    # @example
    #   ProGrammar.config.commands["help"] = MyHelpCommand
    #
    def []=(pattern, command)
      ...
      unless command.is_a?(Class) && command < ProGrammar::Command
        raise TypeError, "command is not a subclass of ProGrammar::Command"
      end
    ...
    #   a subclass of ProGrammar::Command.
    #
