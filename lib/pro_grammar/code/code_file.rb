...
class ProGrammar
  class CodeFile
    ...
    EXTENSIONS = {
      ...
      %w[.rb .ru .irbrc .gemspec .pro_grammarrc .rake] => :ruby
    }.freeze
    ...
    def code
      if @filename == ProGrammar.eval_path
        ProGrammar.line_buffer.drop(1)
      elsif ProGrammar::Method::Patcher.code_for(@filename)
        ProGrammar::Method::Patcher.code_for(@filename)
      else
        path = abs_path
    ...
    # @return [Array] All the paths that contain code that ProGrammar can use for its
    #   API's. Skips directories.
    def code_path
      [from_pwd, from_pro_grammar_init_pwd, *from_load_path]
    end
