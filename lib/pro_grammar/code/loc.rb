...
class ProGrammar
  class Code
    ...
    # instance methods.  `ProGrammar::Code` heavily uses this class.
    #
    # @api private
    # @example
    ...
    class LOC
      ...
      def add_line_number(max_width = 0, color = false)
        ...
        colorized_lineno =
          if color
            ProGrammar::Helpers::BaseHelpers.colorize_code(padded)
          else
            padded
          end
      ...
      def add_marker(marker_lineno)
        tuple[0] =
          if lineno == marker_lineno
            "🐰=>".light_magenta.bold.blink + "#{line}"
          elsif determine_eval_result(line)
            "    #{line}" + " =>".light_magenta + "#{determine_eval_result(line)}".blue 
          else
            "    #{line}"
          end
      end
      ...
      def determine_eval_result(line)
        ...
        ObjectSpace.each_object(ProGrammar) do |instance| 
          if conditional_found || assignment_found
            evaluation = ProGrammar::Evaluation.new(instance)
            return evaluation.evaluate_code(line)
          end
        end
        return false
      end
